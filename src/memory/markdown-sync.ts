 ...
 * - Daily logs: ~/zee/memory/YYYY-MM-DD.md
 * - Entity pages: ~/zee/memory/bank/entities/{entity-name}.md
 *
...
export interface MarkdownSyncConfig {
  /** Base directory for markdown files. Default: ~/zee/memory */
  baseDir?: string;
...
const DEFAULT_BASE_DIR = join(homedir(), "zee", "memory");
...
export class MarkdownSync {
  ...
  private ensureDirs(): void {
    ...
    try {
      const dirs = [
        this.baseDir,
        join(this.baseDir, "bank"),
        join(this.baseDir, "bank", "entities"),
      ];
  ...
  appendToDailyLog(entry: MemoryEntry): void {
    ...
    try {
      ...
      const confidenceTag = this.includeConfidence && entry.confidence !== undefined
        ? ` {${(entry.confidence * 100).toFixed(0)}%}`
        : "";

      lines.push(
        `## ${time} - ${entry.category}${location}${versionTag}${confidenceTag}`,
        "",
        entry.content,
        "",
      );
    ...
    try {
      ...
      if (content.facts.length > 0) {
        ...
        for (const fact of content.facts) {
          const conf = fact.confidence !== undefined
