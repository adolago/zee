...
async function withTempHome<T>(fn: (home: string) => Promise<T>): Promise<T> {
  const base = fs.mkdtempSync(path.join(os.tmpdir(), "zee-agent-"));
...
function mockConfig(
  home: string,
  storePath: string,
  routingOverrides?: Partial<NonNullable<ZeeConfig["routing"]>>,
  agentOverrides?: Partial<NonNullable<ZeeConfig["agent"]>>,
