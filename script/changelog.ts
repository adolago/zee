...
if (!CHANGELOG_REPO) {
  throw new Error("CHANGELOG_REPO is required (owner/repo)")
}
...
export async function getLatestRelease() {
  return fetch(`https://api.github.com/repos/${CHANGELOG_REPO}/releases/latest`)
    .then((res) => {
...
export async function getCommits(from: string, to: string): Promise<Commit[]> {
  ...
  const compare =
    await $`gh api "/repos/${CHANGELOG_REPO}/compare/${fromRef}...${toRef}" --jq '.commits[] | {sha: .sha, login: .author.login, message: .commit.message}'`.text()
...
export async function getContributors(from: string, to: string) {
  ...
  const toRef = to === "HEAD" ? to : to.startsWith("v") ? to : `v${to}`
  const compare =
    await $`gh api "/repos/${CHANGELOG_REPO}/compare/${fromRef}...${toRef}" --jq '.commits[] | {login: .author.login, message: .commit.message}'`.text()
