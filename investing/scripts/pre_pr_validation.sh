...
# Pre-PR Validation Script for Investing
...
if [ ! -f "pyproject.toml" ] && [ ! -f "investing-gui/Cargo.toml" ]; then
    echo -e "${RED}Error: This script must be run from the Investing root directory${NC}"
    exit 1
fi

print_header "Investing Pre-PR Validation"
...
if command -v black &> /dev/null; then
    if black --check investing/ 2>/dev/null; then
        print_pass "Black formatting check passed"
    else
        print_fail "Black formatting issues found. Run: black investing/"
    fi
else
...
if command -v flake8 &> /dev/null; then
    if flake8_output=$(flake8 investing/ 2>&1); then
...
if command -v mypy &> /dev/null; then
    if mypy_output=$(mypy investing/ 2>&1); then
...
if command -v pytest &> /dev/null; then
    ...
    if coverage_output=$(pytest tests/ --cov=investing --cov-report=term-missing 2>&1); then
...
if grep -r "PRIVATE_KEY\|SECRET_KEY\|API_KEY\|PASSWORD" investing/ 2>/dev/null | grep -v ".pyc\|__pycache__" | grep "=.*['\"]"; then
...
python_files=$(find investing -name "*.py" -type f ! -path "*/__pycache__/*" | wc -l)
if [ "$python_files" -gt "0" ]; then
    print_info "Found $python_files Python files"
    # Count files with docstrings (basic check)
    documented_files=$(grep -l '"""' investing/**/*.py 2>/dev/null | wc -l)
