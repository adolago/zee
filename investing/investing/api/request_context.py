"""Request-scoped context for the Investing API.

Routers historically relied on module-level globals (e.g. `_app_state`) or
imports from `investing.api.main` to access the application's state.
...
The context is set by middleware in `investing.api.main_new` for each request.
"""
...
_current_app_state: ContextVar[Optional[Any]] = ContextVar(
    "investing_current_app_state", default=None
)
