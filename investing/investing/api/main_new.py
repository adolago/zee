"""
Investing REST API - Refactored Main Module
==========================================

FastAPI-based REST API for the Investing institutional investment analysis platform.
...
Usage:
    # Development
    uvicorn investing.api.main_new:app --reload --port 8000

    # Production
    uvicorn investing.api.main_new:app --host 0.0.0.0 --port 8000 --workers 4

Environment Variables:
    SEC_IDENTITY: Email for SEC EDGAR API identification
    ZEE_INVESTING_AUTH_JWT_SECRET_KEY: JWT signing key (32+ chars)
    ZEE_INVESTING_AUTH_CORS_ORIGINS: Comma-separated CORS origins
    ZEE_INVESTING_AUTH_RATE_LIMIT_ENABLED: Enable rate limiting (default: true)
"""
...
from investing.api.dependencies import get_container, Container
from investing.api.middleware.security import SecurityHeadersMiddleware
from investing.api.middleware.wide_events import WideEventsMiddleware
from investing.api.request_context import reset_current_app_state, set_current_app_state
from investing.api.routers.registration import register_routers
from investing.api.utils import create_response, get_timestamp, is_debug_mode
...
async def lifespan(app: FastAPI):
    ...
    """
    # Startup
    logger.info("Investing API starting up...")
    ...
    logger.info("Investing API ready")

    yield

    # Shutdown
    logger.info("Investing API shutting down...")
    if owns_container:
    ...
    logger.info("Investing API shutdown complete")
...
def create_app() -> FastAPI:
    ...
    from investing.api.auth import get_auth_settings, RateLimitMiddleware
    ...
    try:
        ...
        logger.critical("ZEE_INVESTING_AUTH_JWT_SECRET_KEY is required for security.")
