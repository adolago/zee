...
Comprehensive Pydantic models with custom validators for Investing API.
...
class InvestingBaseModel(BaseModel):
    """Base Pydantic model for all Investing API models."""
...
class ValidatedPortfolioHolding(InvestingBaseModel):
...
class ValidatedPortfolioRequest(InvestingBaseModel):
...
class ValidatedMoneyFlowRequest(InvestingBaseModel):
...
class ValidatedResearchRequest(InvestingBaseModel):
...
class ValidatedCommoditiesRequest(InvestingBaseModel):
...
class ValidatedOptionsRequest(InvestingBaseModel):
...
class ValidatedMarketData(InvestingBaseModel):
...
class ValidatedPortfolioAnalytics(InvestingBaseModel):
...
class ValidatedValuationMetrics(InvestingBaseModel):
