...
Forwards Investing alerts to Zee's messaging gateway for delivery via Telegram.
Investing has a designated Telegram bot that Zee manages.
...
@dataclass
class InvestingAlert:
    """Investing alert structure."""
    ...
    def to_telegram_message(self) -> str:
        ...
        lines = [
            f"{emoji} {type_icon} **Investing Alert: {self.title}**",
            "",
...
class AlertForwarder:
    """
    Forwards Investing alerts through Zee's messaging gateway.
    ...
    def __init__(
        ...
        self._telegram_chat_id = telegram_chat_id or os.getenv(
            "ZEE_INVESTING_TELEGRAM_CHAT_ID"
        )
        self._enabled = enabled
        self._client: Optional[httpx.AsyncClient] = None

        # Alert queue for batching
        self._alert_queue: List[InvestingAlert] = []
    ...
    def _should_send(self, alert: InvestingAlert) -> bool:
    ...
    async def _send_telegram_message(
        ...
        try:
            ...
            # The daemon routes @investing messages to Investing
            response = await self._client.post(
                f"{self._daemon_url}/session/investing/message",
                json={
                    "message": message,
                    "metadata": {
    ...
    async def _flush_alerts(self) -> None:
        ...
        by_symbol: Dict[str, List[InvestingAlert]] = {}
...
async def send_stanley_alert(
    ...
    """
    Convenience function to send a Investing alert.
