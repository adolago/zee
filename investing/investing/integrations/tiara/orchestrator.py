"""
Investing Orchestrator - Tiara Integration

Integrates Investing's market data streaming and analysis with Tiara's
...
class AgentType(Enum):
    """Investing-specific agent types for Tiara."""
...
class InvestingOrchestrator:
    """
    Orchestrates Investing's market analysis using Tiara's hive-mind.
    ...
    def __init__(
        ...
        """
        Initialize Investing orchestrator.
    ...
    async def initialize_swarm(self) -> str:
        """
        Initialize a Tiara swarm for Investing operations.
        ...
        try:
            ...
            result = await self._run_tiara_command(
                ...
                "--name",
                "investing-market-swarm",
            )

            # Parse swarm ID from result
            self._swarm_id = result.get(
                "swarmId", f"investing-{datetime.now().timestamp()}"
            )
            self._running = True

            logger.info(f"Initialized Investing swarm: {self._swarm_id}")
            return self._swarm_id

        except Exception as e:
            logger.error(f"Failed to initialize swarm: {e}")
            # Fallback to local orchestration
            self._swarm_id = f"investing-local-{datetime.now().timestamp()}"
            self._running = True
            return self._swarm_id
    ...
    def _map_agent_type(self, agent_type: AgentType) -> str:
        """Map Investing agent type to Tiara agent type."""
        mapping = {
    ...
    async def shutdown(self) -> None:
        ...
        logger.info("Investing orchestrator shutdown complete")
...
_orchestrator: Optional[InvestingOrchestrator] = None


def get_orchestrator() -> InvestingOrchestrator:
    """Get or create the global Investing orchestrator."""
    global _orchestrator
    if _orchestrator is None:
        _orchestrator = InvestingOrchestrator()
