 ...
 * The carousel moves right-to-left: enters from right, exits on left, repeats.
 *
 * Animation phases:
 * 1. Enter: blocks appear one-by-one from right edge
...
function getCarouselState(
  ...
  return {
    ...
    isMovingForward: false, // Moving backward (right-to-left)
    activeCount,
    phase: "backward",
  }
}
 ...
 * - HEAD (brightest) is at the left edge of the visible group
...
function getCarouselColorIndex(
  ...
  // Head (index 0) is brightest, trail extends RIGHT (no wrap)
  const trailIndex = charIndex - head
