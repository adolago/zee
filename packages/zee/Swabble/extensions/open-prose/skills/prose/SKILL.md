...
metadata: {"zee":{"emoji":"🪶","homepage":"https://www.prose.md"}}
---
...
- **Task tool** in the upstream spec == Zee `sessions_spawn`
- **File I/O** == Zee `read`/`write`
- **Remote fetch** == Zee `web_fetch` (or `exec` with curl when POST is required)
