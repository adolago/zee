...
`zee.invoke --each`) without adding new Zee code per workflow.
...
This extension depends on Zee internal modules (the embedded agent runner).
It is intended to ship as a **bundled** Zee extension (like `lobster`) and
be enabled via `plugins.entries` + tool allowlists.

It is **not** currently designed to be copied into
`~/.zee/extensions` as a standalone plugin directory.
