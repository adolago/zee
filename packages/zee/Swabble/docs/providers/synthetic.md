---
summary: "Use Synthetic's Anthropic-compatible API in Zee"
read_when:
...
Synthetic exposes Anthropic-compatible endpoints. Zee registers it as the
...
```bash
zee onboard --auth-choice synthetic-api-key
```
...
Note: Zee's Anthropic client appends `/v1` to the base URL, so use
