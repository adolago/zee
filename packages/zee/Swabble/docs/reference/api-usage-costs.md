...
Zee features that can generate provider usage or paid API calls.
...
- `zee status --usage` and `zee channels list` show provider **usage windows**
...
Zee can pick up credentials from:
...
### 8) Model scan / probe
`zee models scan` can probe OpenRouter models and uses `OPENROUTER_API_KEY` when
probing is enabled.
