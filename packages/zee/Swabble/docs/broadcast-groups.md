...
Broadcast groups are evaluated after channel allowlists and group activation rules. In WhatsApp groups, this means broadcasts happen when Zee would normally reply (for example: on mention, depending on your group settings).
...
**Result:** When Zee would reply in this chat, it will run all three agents.
...
Workspace: /Users/pascal/zee-alfred/
...
Workspace: /Users/pascal/zee-baerbel/
Tools: read only
```
...
**Debug:**
```bash
tail -f ~/.zee/logs/gateway.log | grep broadcast
```
