...
read_when:
  - You are building a Zee plugin
  - You need to ship a plugin config schema or debug plugin validation errors
---
# Plugin manifest (zee.plugin.json)

Every plugin **must** ship a `zee.plugin.json` file in the **plugin root**.
Zee uses this manifest to validate configuration **without executing plugin
