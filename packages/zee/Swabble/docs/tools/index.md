---
summary: "Agent tool surface for Zee (browser, canvas, nodes, message, cron) replacing legacy `zee-*` skills"
read_when:
...
You can globally allow/deny tools via `tools.allow` / `tools.deny` in `zee.json`
...
- If `tools.allow` only references unknown or unloaded plugin tool names, Zee logs a warning and ignores the allowlist so core tools stay available.
...
- `host=node` can target a headless node host (`zee node run`).
...
Notes:
- Requires a Brave API key (recommended: `zee configure --section web`, or set `BRAVE_API_KEY`).
...
### `browser`
Control the dedicated zee browser.
...
Core actions:
- `restart` (authorizes + sends `SIGUSR1` for in-process restart; `zee gateway` restart in-place)
