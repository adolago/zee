---
summary: "Chrome extension: let Zee drive your existing Chrome tab"
read_when:
...
The Zee Chrome extension lets the agent control your **existing Chrome tabs** (your normal Chrome window) instead of launching a separate zee-managed Chrome profile.
...
Zee then controls the attached tab through the normal `browser` tool surface (selecting the right profile).
...
The extension ships inside the Zee release (npm package) as static files. There is no separate “build” step.

After upgrading Zee:
- Re-run `zee browser extension install` to refresh the installed files under your Zee state directory.
- Chrome → `chrome://extensions` → click “Reload” on the extension.

## Use it (no extra config)

Zee ships with a built-in browser profile named `chrome` that targets the extension relay on the default port.
...
- Open the tab you want Zee to control.
...
- `ON`: attached; Zee can drive that tab.
...
`zee browser extension path` prints the **installed** on-disk directory containing the extension files.

The CLI intentionally does **not** print a `node_modules` path. Always run `zee browser extension install` first to copy the extension to a stable location under your Zee state directory.
...
- **This is not isolated** like the dedicated zee-managed profile.
