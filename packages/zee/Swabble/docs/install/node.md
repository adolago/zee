...
read_when:
  - "You installed Zee but `zee` is “command not found”"
...
Zee’s runtime baseline is **Node 22+**.

If you can run `npm install -g zee@latest` but later see `zee: command not found`, it’s almost always a **PATH** issue: the directory where npm puts global binaries isn’t on your shell’s PATH.
