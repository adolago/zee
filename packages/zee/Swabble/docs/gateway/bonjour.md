...
Zee uses Bonjour (mDNS / DNS‑SD) as a **LAN‑only convenience** to discover
...
- serve `zee.internal.` from `~/.zee/dns/zee.internal.db`

Validate from a tailnet‑connected machine:

```bash
dns-sd -B _zee-gw._tcp zee.internal.
dig @<TAILNET_IPV4> -p 53 _zee-gw._tcp.zee.internal PTR +short
```
...
Only the Gateway advertises `_zee-gw._tcp`.
...
- Browse instances:
  ```bash
  dns-sd -B _zee-gw._tcp local.
  ```
- Resolve one instance (replace `<instance>`):
  ```bash
  dns-sd -L "<instance>" _zee-gw._tcp local.
  ```
