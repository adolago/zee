...
Goal: let Zee sit in WhatsApp groups, wake up only when pinged, and keep that thread separate from the personal DM session.
...
Notes:
- The regexes are case-insensitive; they cover a display-name ping like `@zee` and the raw number with or without `+`/spaces.
...
## How to use
1) Add your WhatsApp account (the one running Zee) to the group.
2) Say `@zee …` (or include the number). Only allowlisted senders can trigger it unless you set `groupPolicy: "open"`.
...
- Manual smoke:
  - Send an `@zee` ping in the group and confirm a reply that references the sender name.
