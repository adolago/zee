---
summary: "How Zee memory works (workspace files + automatic memory flush)"
read_when:
...
Zee memory is **plain Markdown in the agent workspace**. The files are the
...
`~/zee`). See [Agent workspace](/concepts/agent-workspace) for the full layout.
...
When a session is **close to auto-compaction**, Zee triggers a **silent,
...
Zee can build a small vector index over `MEMORY.md` and `memory/*.md` (plus
...
- Index storage: per-agent SQLite at `~/.zee/memory/<agentId>.sqlite` (configurable via `agents.defaults.memorySearch.store.path`, supports `{agentId}` token).
...
- Reindex triggers: the index stores the embedding **provider/model + endpoint fingerprint + chunking params**. If any of those change, Zee automatically resets and reindexes the entire store.

### Hybrid search (BM25 + vector)

When enabled, Zee combines:
- **Vector similarity** (semantic match, wording can differ)
- **BM25 keyword relevance** (exact tokens like IDs, env vars, code symbols)

If full-text search is unavailable on your platform, Zee falls back to vector-only search.
...
Zee can cache **chunk embeddings** in SQLite so reindexing and frequent updates (especially session transcripts) don't re-embed unchanged text.
...
- Session logs live on disk (`~/.zee/agents/<agentId>/sessions/*.jsonl`). Any process/user with filesystem access can read them, so treat disk access as the trust boundary. For stricter isolation, run agents under separate OS users or hosts.
...
When the sqlite-vec extension is available, Zee stores embeddings in a
...
- If the sqlite-vec extension is missing or fails to load, Zee logs the
