---
summary: "OAuth in Zee: token exchange, storage, and multi-account patterns"
read_when:
...
Zee supports “subscription auth” via OAuth for providers that offer it (notably **OpenAI Codex (ChatGPT OAuth)**). For Anthropic subscriptions, use the **setup-token** flow. This page explains:

- how the OAuth **token exchange** works (PKCE)
- where tokens are **stored** (and why)
- how to handle **multiple accounts** (profiles + per-session overrides)

Zee also supports **provider plugins** that ship their own OAuth or API‑key
flows. Run them via:

```bash
zee models auth login --provider <id>
```
...
Run `claude setup-token` on any machine, then paste it into Zee:
...
2) paste the token into Zee
3) store as a token auth profile (no refresh)

The wizard path is `zee onboard` → auth choice `setup-token` (Anthropic).
...
Wizard path is `zee onboard` → auth choice `openai-codex`.
...
- `zee channels list --json` (shows `auth[]`)
