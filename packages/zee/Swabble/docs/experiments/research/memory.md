---
summary: "Research notes: offline memory system for Zee workspaces (Markdown source-of-truth + derived index)"
read_when:
  - Designing workspace memory (~/zee) beyond daily Markdown logs
...
Target: Zee-style workspace (`agents.defaults.workspace`, default `~/zee`) where “memory” is stored as one Markdown file per day (`memory/YYYY-MM-DD.md`) plus a small set of stable files (e.g. `memory.md`, `SOUL.md`).
...
Keep `~/zee` as canonical human-readable memory.
...
- `memory.md` remains “small + core-ish”: the things you want Zee to see every session.
