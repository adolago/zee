...
read_when:
  ...
  - Investigating tsx/esbuild loader crashes in Zee
---

# Node + tsx "__name is not a function" crash

## Summary
Running Zee via Node with `tsx` fails at startup with:

```
[zee] Failed to start CLI: TypeError: __name is not a function
...
- Before that (Bun path), `zee status` and `gateway:watch` worked.
...
- Use Node + tsc watch, then run compiled output:
  ...
  node --watch zee.mjs status
  ```
- Confirmed locally: `pnpm exec tsc -p tsconfig.json` + `node zee.mjs status` works on Node 25.
