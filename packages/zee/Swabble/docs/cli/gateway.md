...
The Gateway is Zee’s WebSocket server (channels, nodes, sessions, hooks).
...
```bash
zee gateway probe --ssh user@gateway-host
```
...
`gateway discover` scans for Gateway beacons (`_zee-gw._tcp`).
