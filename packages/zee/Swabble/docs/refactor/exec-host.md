...
- Split execution into a **headless runner service** with optional CLI/TUI via local IPC.
...
- **Runner:** headless system service; CLI/TUI hosts a Unix socket for approvals.
- **Node identity:** use existing `nodeId`.
- **Socket auth:** Unix socket + token (cross-platform); split later if needed.
- **Node host state:** `~/.zee/node.json` (node id + pairing token).
- **UI exec host:** run `system.run` inside the local CLI/TUI; node host service forwards requests over local IPC.
...
- Add Unix socket prompt bridge to CLI/TUI.
- Wire `askFallback`.
...
### Phase 5: UI polish
- CLI/TUI: allowlist editor, per-agent switcher, ask policy UI.
