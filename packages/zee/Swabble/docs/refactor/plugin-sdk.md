...
Delivery:
- Publish as `@zee/plugin-sdk` (or export from core under `zee/plugin-sdk`).
- Semver with explicit stability guarantees.

### 2) Plugin Runtime (execution surface, injected)
Scope: everything that touches core runtime behavior.
Accessed via `ZeePluginApi.runtime` so plugins never import `src/**`.
...
export type PluginRuntime = {
  channel: {
    text: {
      ...
      resolveTextChunkLimit(cfg: ZeeConfig, channel: string, accountId?: string): number;
      hasControlCommand(text: string, cfg: ZeeConfig): boolean;
    };
    reply: {
    ...
    mentions: {
      buildMentionRegexes(cfg: ZeeConfig, agentId?: string): RegExp[];
      matchesMentionPatterns(text: string, regexes: RegExp[]): boolean;
    };
    groups: {
      resolveGroupPolicy(cfg: ZeeConfig, channel: string, accountId: string, groupId: string): {
    ...
    debounce: {
      ...
      resolveInboundDebounceMs(cfg: ZeeConfig, channel: string): number;
    };
    commands: {
  ...
  state: {
    resolveStateDir(cfg: ZeeConfig): string;
  };
};
```
...
### Phase 0: scaffolding
- Introduce `@zee/plugin-sdk`.
- Add `api.runtime` to `ZeePluginApi` with the surface above.
...
- Plugins declare a required runtime range (e.g., `zeeRuntime: ">=2026.2.0"`).
