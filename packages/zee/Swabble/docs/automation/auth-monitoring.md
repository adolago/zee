...
Zee exposes OAuth expiry health via `zee models status`. Use that for
...
- `scripts/claude-auth-status.sh` now uses `zee models status --json` as the
  source of truth (falling back to direct file reads if the CLI is unavailable),
  so keep `zee` on `PATH` for timers.
