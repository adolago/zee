...
This document describes how the Zee ACP (Agent Client Protocol) bridge works,
how it maps ACP sessions to Gateway sessions, and how IDEs should invoke it.

## Overview

`zee acp` exposes an ACP agent over stdio and forwards prompts to a running
Zee Gateway over WebSocket. It keeps ACP session ids mapped to Gateway
...
drive a Zee Gateway session.

Quick steps:

1. Run a Gateway (local or remote).
2. Configure the Gateway target (`gateway.remote.url` + auth) or pass flags.
3. Point the IDE to run `zee acp` over stdio.

Example config:

```bash
zee config set gateway.remote.url wss://gateway-host:18789
zee config set gateway.remote.token <token>
```

Example run:

```bash
zee acp --url wss://gateway-host:18789 --token <token>
```
...
```bash
zee acp --session agent:main:main
zee acp --session agent:design:main
zee acp --session agent:qa:bug-123
```
...
In Zed, open the Agent panel and select “Zee ACP” to start a thread.

## Execution Model

- ACP client spawns `zee acp` and speaks ACP messages over stdio.
...
```bash
zee acp --session agent:main:main
zee acp --session-label "support inbox"
zee acp --reset-session
```
...
`zee acp` resolves the Gateway URL and auth from CLI flags or config:
