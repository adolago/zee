  ...
  const applyHotReload = async (
    ...
    if (plan.restartGmailWatcher) {
      ...
      if (!isTruthyEnvValue(process.env.ZEE_SKIP_GMAIL_WATCHER)) {
        ...
        params.logHooks.info("skipping gmail watcher restart (ZEE_SKIP_GMAIL_WATCHER=1)");
      }
    }

    if (plan.restartChannels.size > 0) {
      if (isTruthyEnvValue(process.env.ZEE_SKIP_CHANNELS)) {
        params.logChannels.info("skipping channel reload (ZEE_SKIP_CHANNELS=1)");
      } else {
