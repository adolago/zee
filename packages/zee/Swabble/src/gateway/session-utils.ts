...
function resolveIdentityAvatarUrl(cfg: ZeeConfig, agentId: string, avatar: string | undefined): string | undefined {
...
export function listAgentsForGateway(cfg: ZeeConfig): {
  defaultId: string
  mainKey: string
  scope: SessionScope
...
export function resolveSessionModelRef(cfg: ZeeConfig, entry?: SessionEntry): { provider: string; model: string } {
  const resolved = resolveConfiguredModelRef({
    cfg,
    defaultProvider: DEFAULT_PROVIDER,
    defaultModel: DEFAULT_MODEL,
