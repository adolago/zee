...
`;
    ...
    const parsed = JSON5.parse(result.metadata ?? "") as { zee?: { emoji?: string } };
    expect(parsed.zee?.emoji).toBe("disk");
  });
...
`;
    ...
    const parsed = JSON5.parse(result.metadata ?? "") as { zee?: { events?: string[] } };
