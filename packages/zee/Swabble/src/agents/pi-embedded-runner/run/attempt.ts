...
import { resolveZeeDocsPath } from "../../docs-path.js";
...
import { getDmHistoryLimitFromSessionKey, limitHistoryBytes, limitHistoryTurns } from "../history.js";
...
export async function runEmbeddedAttempt(
  ...
  try {
    ...
    const docsPath = await resolveZeeDocsPath({
      workspaceDir: effectiveWorkspace,
    ...
    try {
      ...
      try {
        ...
        const limited = limitHistoryBytes(turnLimited, undefined);
      ...
      const runtimeChannel = (params.messageChannel ?? params.messageProvider ?? "")
        .trim()
        .toLowerCase();
      const effectiveReasoningMode = runtimeChannel === "whatsapp"
        ? "off"
