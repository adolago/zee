...
import { parseFrontmatter, resolveZeeMetadata, resolveHookInvocationPolicy } from "./frontmatter.js";
import type {
  Hook,
...
export function loadHookEntriesFromDir(params: {
  ...
  return hooks.map((hook) => {
    ...
    const entry: HookEntry = {
      ...
      frontmatter,
      metadata: resolveZeeMetadata(frontmatter),
...
function loadHookEntries(
  ...
  const extraHooks = extraDirs.flatMap((dir) => {
    ...
    return loadHooksFromDir({
      dir: resolved,
      source: "zee-workspace", // Extra dirs treated as workspace
    });
  });
