...
export class DirectoryCache<T> {
  ...
  private lastConfigRef: ZeeConfig | null = null;

  constructor(private readonly ttlMs: number) {}

  get(key: string, cfg: ZeeConfig): T | undefined {
  ...
  set(key: string, value: T, cfg: ZeeConfig): void {
    this.resetIfConfigChanged(cfg);
  ...
  clear(cfg?: ZeeConfig): void {
    this.cache.clear();
    if (cfg) this.lastConfigRef = cfg;
  }

  private resetIfConfigChanged(cfg: ZeeConfig): void {
