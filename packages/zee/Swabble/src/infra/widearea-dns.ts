...
function renderZone(opts: WideAreaGatewayZoneOpts & { serial: number }): string {
  ...
  const instanceLabel = dnsLabel(opts.instanceLabel ?? `${hostname}-gateway`, "zee-gw");
  ...
  records.push(`_zee-gw._tcp IN PTR ${instanceLabel}._zee-gw._tcp`);
  records.push(`${instanceLabel}._zee-gw._tcp IN SRV 0 0 ${opts.gatewayPort} ${hostLabel}`);
  records.push(`${instanceLabel}._zee-gw._tcp IN TXT ${txt.map(txtQuote).join(" ")}`);
