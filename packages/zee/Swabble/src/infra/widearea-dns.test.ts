...
describe("wide-area DNS-SD zone rendering", () => {
  it("renders a zee.internal zone with gateway PTR/SRV/TXT records", () => {
    const txt = renderWideAreaGatewayZoneText({
      serial: 2025121701,
      gatewayPort: 18789,
      displayName: "Gateway Host (Zee)",
    ...
    expect(txt).toContain(`_zee-gw._tcp IN PTR studio-london._zee-gw._tcp`);
    expect(txt).toContain(`studio-london._zee-gw._tcp IN SRV 0 0 18789 studio-london`);
    expect(txt).toContain(`displayName=Gateway Host (Zee)`);
    expect(txt).toContain(`gatewayPort=18789`);
    expect(txt).toContain(`sshPort=22`);
    expect(txt).toContain(`cliPath=/usr/local/bin/zee`);
  });
  ...
  it("includes tailnetDns when provided", () => {
    const txt = renderWideAreaGatewayZoneText({
      ...
      displayName: "Gateway Host (Zee)",
      tailnetIPv4: "100.123.224.76",
      tailnetDns: "peters-gateway-host-1.sheep-coho.ts.net",
      hostLabel: "studio-london",
      instanceLabel: "studio-london",
    });

    expect(txt).toContain(`tailnetDns=peters-gateway-host-1.sheep-coho.ts.net`);
  });
});
