...
describe("media store", () => {
  ...
  const snapshotEnv = () => {
    for (const key of ["HOME", "USERPROFILE", "HOMEDRIVE", "HOMEPATH", "ZEE_STATE_DIR"]) {
  ...
  beforeAll(async () => {
    snapshotEnv();
    home = await fs.mkdtemp(path.join(os.tmpdir(), "zee-test-home-"));
    process.env.HOME = home;
    process.env.USERPROFILE = home;
    process.env.ZEE_STATE_DIR = path.join(home, ".zee");
    ...
    await fs.mkdir(path.join(home, ".zee"), { recursive: true });
  ...
  it("creates and returns media directory", async () => {
    await withTempStore(async (store, home) => {
      ...
      expect(path.normalize(dir)).toContain(`${path.sep}.zee${path.sep}media`);
