...
export type HealthSummary = {
  /**
   * Convenience top-level flag for UIs that only need a binary
...
const debugHealth = (...args: unknown[]) => {
  if (isTruthyEnvValue(process.env.ZEE_DEBUG_HEALTH)) {
...
export async function healthCommand(
  opts: { json?: boolean; timeoutMs?: number; verbose?: boolean; config?: ZeeConfig },
  runtime: RuntimeEnv,
) {
  ...
  if (opts.json) {
    ...
    const debugEnabled = isTruthyEnvValue(process.env.ZEE_DEBUG_HEALTH);
    if (opts.verbose) {
