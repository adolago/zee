...
function resolveDefaultAccountId(cfg: ZeeConfig, provider: ChannelId): string {
  const plugin = getChannelPlugin(provider);
  if (!plugin) return DEFAULT_ACCOUNT_ID;
  return resolveChannelDefaultAccountId({ plugin, cfg });
}

function shouldShowProviderEntry(entry: ProviderAccountStatus, cfg: ZeeConfig): boolean {
...
export function summarizeBindings(cfg: ZeeConfig, bindings: AgentBinding[]): string[] {
