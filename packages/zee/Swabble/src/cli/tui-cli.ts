...
export function registerTuiCli(program: Command) {
  program
    .command("tui")
    .description("Open the Zee terminal UI")
    .option("--url <url>", "Zee daemon URL (http[s]://host:port)")
    .option("--token <token>", "Legacy gateway token (ignored; use zee auth)")
    .option("--password <password>", "Zee auth password (if enabled)")
