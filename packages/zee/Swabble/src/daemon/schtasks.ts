...
function resolveTaskName(env: Record<string, string | undefined>): string {
  const override = env.ZEE_WINDOWS_TASK_NAME?.trim();
  if (override) return override;
  return resolveGatewayWindowsTaskName(env.ZEE_PROFILE);
}

export function resolveTaskScriptPath(env: Record<string, string | undefined>): string {
  const override = env.ZEE_TASK_SCRIPT?.trim();
  if (override) return override;
  const scriptName = env.ZEE_TASK_SCRIPT_NAME?.trim() ?? "gateway.cmd";
