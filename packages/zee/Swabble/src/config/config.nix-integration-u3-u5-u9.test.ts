...
describe("Nix integration (U3, U5, U9)", () => {
  ...
  describe("U5: CONFIG_PATH and STATE_DIR env var overrides", () => {
    it("STATE_DIR defaults to ~/.zee when env not set", async () => {
      await withEnvOverride(
        { ZEE_STATE_DIR: undefined },
        async () => {
          const { STATE_DIR } = await import("./config.js");
          expect(STATE_DIR).toMatch(/\.zee$/);
        },
      );
    });

    it("STATE_DIR respects ZEE_STATE_DIR override", async () => {
      await withEnvOverride(
    ...
    it("CONFIG_PATH defaults to ~/.zee/zee.json when env not set", async () => {
      await withEnvOverride(
        {
        ...
        async () => {
          ...
          expect(CONFIG_PATH).toMatch(/\.zee[\\/]zee\.json$/);
        },
      );
    });

    it("CONFIG_PATH respects ZEE_CONFIG_PATH override", async () => {
      await withEnvOverride(
        { ZEE_CONFIG_PATH: "/nix/store/abc/zee.json" },
        async () => {
          const { CONFIG_PATH } = await import("./config.js");
          expect(CONFIG_PATH).toBe(path.resolve("/nix/store/abc/zee.json"));
        },
      );
    });

    it("CONFIG_PATH expands ~ in ZEE_CONFIG_PATH override", async () => {
      await withTempHome(async (home) => {
        await withEnvOverride(
          { ZEE_CONFIG_PATH: "~/.zee/custom.json" },
          async () => {
            const { CONFIG_PATH } = await import("./config.js");
            expect(CONFIG_PATH).toBe(path.join(home, ".zee", "custom.json"));
          },
        );
      });
    });
    ...
    it("CONFIG_PATH uses STATE_DIR when only state dir is overridden", async () => {
      await withEnvOverride(
        ...
        async () => {
          const { CONFIG_PATH } = await import("./config.js");
          expect(CONFIG_PATH).toBe(path.join(path.resolve("/custom/state"), "zee.json"));
        },
      );
    });
  });
    ...
    it("expands ~ in common path-ish config fields", async () => {
      await withTempHome(async (home) => {
          ...
          JSON.stringify(
            {
              ...
              channels: {
                whatsapp: {
                  accounts: {
                    personal: {
                      authDir: "~/.zee/credentials/wa-personal",
                    },
                  },
                },
              },
        ...
        expect(cfg.agents?.list?.[0]?.agentDir).toBe(
          path.join(home, ".zee", "agents", "main"),
        );
        expect(cfg.agents?.list?.[0]?.sandbox?.workspaceRoot).toBe(path.join(home, "sandbox-root"));
        expect(cfg.channels?.whatsapp?.accounts?.personal?.authDir).toBe(
          path.join(home, ".zee", "credentials", "wa-personal"),
        );
      });
    });
  });
  ...
  // U9 is intentionally omitted here: legacy channel config was removed.
});
