...
function decoratedMarkerPath(userDataDir: string) {
  return path.join(userDataDir, ".zee-profile-decorated");
}
 ...
 */
export function decorateZeeProfile(
  userDataDir: string,
  opts?: { name?: string; color?: string },
) {
  const desiredName = opts?.name ?? DEFAULT_ZEE_BROWSER_PROFILE_NAME;
  const desiredColor = (opts?.color ?? DEFAULT_ZEE_BROWSER_COLOR).toUpperCase();
