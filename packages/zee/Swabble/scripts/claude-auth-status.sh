...
# Checks both Claude Code and Zee auth status

set -euo pipefail

CLAUDE_CREDS="$HOME/.claude/.credentials.json"
ZEE_AUTH="$HOME/.zee/agents/main/agent/auth-profiles.json"
...
fetch_models_status_json() {
    zee models status --json 2>/dev/null || true
}
...
if [ "$OUTPUT_MODE" = "json" ]; then
    ...
    zee_status=$(check_zee_auth 2>/dev/null || true)

    claude_expires=0
    zee_expires=0
    if [ "$USE_JSON" -eq 1 ]; then
        claude_expires=$(json_expires_for_claude_cli)
        zee_expires=$(json_expires_for_anthropic_any)
    else
        claude_expires=$(jq -r '.claudeAiOauth.expiresAt // 0' "$CLAUDE_CREDS" 2>/dev/null || echo "0")
        zee_expires=$(jq -r '.profiles["anthropic:default"].expires // 0' "$ZEE_AUTH" 2>/dev/null || echo "0")
    fi
    ...
    jq -n \
        ...
        '{
            ...
            zee: {status: $bs, expires_at_ms: ($be | tonumber)},
...
if [ "$OUTPUT_MODE" = "simple" ]; then
    ...
    if [[ "$claude_status" == EXPIRED* ]] || [[ "$claude_status" == MISSING* ]]; then
        echo "CLAUDE_EXPIRED"
        exit 1
    elif [[ "$zee_status" == EXPIRED* ]] || [[ "$zee_status" == MISSING* ]]; then
        echo "ZEE_EXPIRED"
        exit 1
    elif [[ "$claude_status" == EXPIRING* ]]; then
        echo "CLAUDE_EXPIRING"
        exit 2
    elif [[ "$zee_status" == EXPIRING* ]]; then
        echo "ZEE_EXPIRING"
        exit 2
    else
        echo "OK"
...
echo ""
echo "Zee Auth (~/.zee/agents/main/agent/auth-profiles.json):"
if [ "$USE_JSON" -eq 1 ]; then
...
if [ "$expires" -le 0 ] && [ "$api_keys" -gt 0 ]; then
    ...
    echo -e "  Status: ${RED}NOT FOUND${NC}"
    echo "  Note: Run 'zee doctor --yes' to sync from Claude Code"
else
    now_ms=$(( $(date +%s) * 1000 ))
...
echo ""
echo "=== Service Status ==="
if systemctl --user is-active zee >/dev/null 2>&1; then
    echo -e "Zee service: ${GREEN}running${NC}"
else
    echo -e "Zee service: ${RED}NOT running${NC}"
fi
