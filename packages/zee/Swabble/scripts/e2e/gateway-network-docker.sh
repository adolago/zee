...
IMAGE_NAME="zee-gateway-network-e2e"

PORT="18789"
TOKEN="e2e-$(date +%s)-$$"
NET_NAME="zee-net-e2e-$$"
GW_NAME="zee-gateway-e2e-$$"
...
echo "Starting gateway container..."
	docker run --rm -d \
	  --name "$GW_NAME" \
	  --network "$NET_NAME" \
	  -e "ZEE_GATEWAY_TOKEN=$TOKEN" \
	  -e "ZEE_SKIP_CHANNELS=1" \
	  -e "ZEE_SKIP_GMAIL_WATCHER=1" \
	  -e "ZEE_SKIP_CRON=1" \
	  -e "ZEE_SKIP_CANVAS_HOST=1" \
	  "$IMAGE_NAME" \
