...
#
# Suggested cron: */30 * * * * /home/admin/zee/scripts/auth-monitor.sh
#
# Environment variables:
#   NOTIFY_PHONE - Phone number to send Zee notification (e.g., +1234567890)
#   NOTIFY_NTFY  - ntfy.sh topic for push notifications (e.g., zee-alerts)
...
STATE_FILE="$HOME/.zee/auth-monitor-state"
...
send_notification() {
    ...
    # Send via Zee if phone configured and auth still valid
    if [ -n "$NOTIFY_PHONE" ]; then
        # Check if we can still use zee
        if "$SCRIPT_DIR/claude-auth-status.sh" simple 2>/dev/null | grep -q "OK\|EXPIRING"; then
            echo "Sending via Zee to $NOTIFY_PHONE..."
            zee send --to "$NOTIFY_PHONE" --message "$message" 2>/dev/null || true
        fi
    fi
...
if [ "$DIFF_MS" -lt 0 ]; then
    send_notification "Claude Code auth EXPIRED! Zee is down. Run: claude setup-token" "urgent"
    exit 1
