...
export function buildFollowupExecutionReminder(input: {
  ...
  const planModeLine =
    input.surface === "whatsapp" || input.surface === "telegram"
      ? "If PLAN mode blocks this action, ask the user to switch to ACCEPT mode (for example `/accept <PIN>` when required) before executing instead of claiming the integration is unavailable."
