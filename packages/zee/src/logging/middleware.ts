 ...
 */
export async function withSessionLogging<T>(sessionId: string, fn: () => Promise<T>): Promise<T> {
 ...
 */
export function logAgentIteration(iterationNumber: number, metadata?: Record<string, unknown>): void {
 ...
 */
export function createOperationLogger(operation: string, metadata?: Record<string, unknown>): ILogger {
