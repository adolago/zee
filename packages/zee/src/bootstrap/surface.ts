...
async function registerWhatsAppSurface(waConfig?: SurfaceBootstrapConfig["whatsApp"]): Promise<void> {
  if (!router) return
  ...
  const surface = createMessagingSurface(handler, {
    platform: "whatsapp",
    allowedSenders: [...(waConfig?.operators ?? []), ...(waConfig?.allowedNumbers ?? [])],
    groups: {
...
async function resolveSessionId(senderId: string, isGroup: boolean, groupId?: string): Promise<string> {
  ...
  const title = isGroup && groupId ? `WhatsApp Group: ${groupId}` : `WhatsApp: ${senderId}`
...
function createEngineMessageHandler(): MessageHandler {
  return async (message, context) => {
    const sessionId = await resolveSessionId(context.senderId, context.isGroup, context.threadId)
