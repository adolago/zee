...
function writeOsc52(text: string): void {
  ...
  // tmux and screen require DCS passthrough wrapping. tmux with set-clipboard
  // on handles the raw sequence itself, so send both raw and wrapped there.
  const passthrough = `\x1bPtmux;\x1b${osc52}\x1b\\`
  const sequence = process.env["TMUX"] ? osc52 + passthrough : process.env["STY"] ? passthrough : osc52
