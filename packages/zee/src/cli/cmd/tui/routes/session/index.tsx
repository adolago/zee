...
const context = createContext<{
  ...
  showDetails: () => boolean
  showToolDetailsForMessage: (message: AssistantMessage, parts?: Part[]) => boolean
...
export function Session() {
  ...
  const showToolDetailsForMessage = (message: AssistantMessage, parts?: Part[]) => {
    const toolParts = (parts ?? sync.data.part[message.id] ?? []).filter(
    ...
    if (message.time.completed || message.error) return false

    const status = sessionStatus()
    const phase = status?.type === "busy" ? status.streamHealth?.phase : undefined
    return phase === "tool_calling"
  }
  ...
  createEffect(() => {
    ...
    return exit.message.set([``, `  \x1b[2m${title}\x1b[0m`, `  \x1b[2mzee -s ${session()?.id}\x1b[0m`, ``].join("\n"))
  })
...
function UserMessage(props: {
  ...
  const color = createMemo(() => theme.accent)
      ...
      <Show when={text()}>
        ...
        >
          ...
          <box flexDirection="column" flexGrow={1}>
            <text fg={color()} attributes={TextAttributes.ITALIC}>
              {text()?.text}
            </text>
...
function ToolPart(props: { last: boolean; part: ToolPart; message: AssistantMessage }) {
  const ctx = use()
  const sync = useSync()
  const showToolDetails = createMemo(() => ctx.showToolDetailsForMessage(props.message))

  // In collapsed mode, keep failed tools visible but hide completed details.
            ...
            <Show when={current()}>
              {(c) => (
                <text style={{ fg: c().state.status === "error" ? theme.error : theme.textMuted }}>
                  └ {Locale.titlecase(c().tool)} {c().state.status === "completed" ? c().state.title : ""}
                </text>
              )}
            </Show>
          </box>
