...
function draw(lines: string[], options: BoxOptions = {}): string {
  ...
  const contentWidth =
    options.width ?? Math.max(...lines.map((l) => visualWidth(l)), title ? visualWidth(title) + 4 : 0)
  ...
  if (title) {
    ...
    result.push(`${borderColor}${borders.topLeft}${borders.horizontal.repeat(innerWidth)}${borders.topRight}${reset}`)
  }
