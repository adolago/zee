...
export namespace Installation {
  ...
  export async function method() {
    if (process.execPath.includes(path.join(".zee", "bin"))) return "curl"
  ...
  export async function upgrade(method: Method, target: string) {
    ...
    switch (method) {
      case "curl":
        cmd = $`curl -fsSL https://raw.githubusercontent.com/adolago/zee/main/install | bash`.env({
          ...process.env,
          VERSION: target,
        })
        break
      ...
      case "choco":
        cmd = $`choco upgrade zee --version ${target} -y`
        break
      case "scoop":
        cmd = $`scoop update zee`
        break
      default:
  ...
  export async function latest(installMethod?: Method) {
    ...
    return fetch("https://api.github.com/repos/adolago/zee/releases/latest")
      .then((res) => {
        if (!res.ok) throw new Error(res.statusText)
        return res.json()
      })
