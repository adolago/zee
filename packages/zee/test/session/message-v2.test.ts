...
// Mock model for testing - matches the default assistantInfo provider/model
...
describe("session.message-v2.toModelMessage", () => {
  ...
  test("omits provider metadata when assistant model differs", async () => {
    ...
    const input: MessageV2.WithParts[] = [
      ...
      {
        // This assistant message was generated by a different provider/model
    ...
    // When model differs, provider metadata should NOT be included to avoid 400 errors
    expect(await MessageV2.toModelMessage(input, model)).toStrictEqual([
      {
        role: "user",
        content: [{ type: "text", text: "run tool" }],
      },
      {
        role: "assistant",
        content: [
          // Note: no providerOptions here since model differs
          { type: "text", text: "done" },
          {
            type: "tool-call",
            toolCallId: "call-1",
  ...
  test("converts pending/running tool calls to error results to prevent dangling tool_use", async () => {
    ...
    const result = await MessageV2.toModelMessage(input, model)
