...
test("loads config from .zee directory", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
...
test("loads agents from .zee/agents (plural)", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
...
test("loads commands from .zee/command (singular)", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const agentCoreDir = path.join(dir, ".zee")
      await fs.mkdir(agentCoreDir, { recursive: true })
...
test("loads commands from .zee/commands (plural)", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const agentCoreDir = path.join(dir, ".zee")
      await fs.mkdir(agentCoreDir, { recursive: true })
...
test("merges plugin arrays from global and local configs", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      // Create a nested project structure with local .zee config
      const projectDir = path.join(dir, "project")
      const opencodeDir = path.join(projectDir, ".zee")
      await fs.mkdir(opencodeDir, { recursive: true })
...
test("local .zee config can override MCP from project config", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      ...
      // Local .zee directory config enables it
      const opencodeDir = path.join(dir, ".zee")
      await fs.mkdir(opencodeDir, { recursive: true })
      await Bun.write(
        path.join(opencodeDir, "zee.json"),
...
describe("deduplicatePlugins", () => {
  ...
  test("prefers local file over npm package with same name", () => {
    const plugins = ["oh-my-opencode@2.4.3", "file:///project/.zee/plugin/oh-my-opencode.js"]

    const result = Config.deduplicatePlugins(plugins)

    expect(result.length).toBe(1)
    expect(result[0]).toBe("file:///project/.zee/plugin/oh-my-opencode.js")
  })
