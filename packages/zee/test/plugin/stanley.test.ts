...
class StanleyApiError extends Error {
  constructor(
    message: string,
...
class StanleyNetworkError extends Error {
...
class StanleyTimeoutError extends Error {
...
class StanleyNotRunningError extends Error {
  constructor(public readonly baseUrl: string) {
    super(`Stanley API not running at ${baseUrl}`)
    this.name = "StanleyNotRunningError"
  }
}

class StanleyDaemonError extends Error {
  constructor(message: string) {
    super(message)
...
const { StanleyPlugin } = await import("../../src/plugin/stanley")
...
async function withPlugin(run: (hooks: Awaited<ReturnType<typeof StanleyPlugin>>) => Promise<void>) {
  const hooks = await StanleyPlugin({} as any)
  try {
    await run(hooks)
  } finally {
...
describe("plugin.stanley", () => {
  ...
  test("returns actionable error when connection fails", async () => {
    state.connectError = new StanleyNetworkError("connection refused")
    ...
    await withPlugin(async (hooks) => {
      ...
      expect(result).toContain("Error: Unable to reach Stanley API")
  ...
  test("retries Brazilian ticker with .SA suffix for research valuation", async () => {
    state.researchBehavior.set("valuation:ALPA4", new StanleyApiError("symbol not found", 404, "missing"))
  ...
  test("retries Brazilian ticker with .SA suffix for market quote", async () => {
    ...
    await withPlugin(async (hooks) => {
      const market = hooks.tool?.stanley_market
      expect(market).toBeDefined()
      if (!market) throw new Error("stanley_market tool missing")
  ...
  test("does not retry .SA fallback on non-symbol network failures", async () => {
    state.researchBehavior.set("valuation:ALPA4", new StanleyNetworkError("socket closed"))
