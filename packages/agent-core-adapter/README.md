...
```bash
bun add @agent-core/agent-core-adapter
```
...
@import "@agent-core/agent-core-adapter/theme.css";
```
...
```tsx
import { ThemeProvider } from "@agent-core/agent-core-adapter"
...
```typescript
import { createAdapter } from "@agent-core/agent-core-adapter"
...
1. Add dependency to `packages/app/package.json`:
   ...
   {
     "dependencies": {
       "@agent-core/agent-core-adapter": "workspace:*"
     }
   }
   ```
