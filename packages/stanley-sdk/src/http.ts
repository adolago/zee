...
export class HttpTransport {
  ...
  async request<T>(endpoint: string, options: RequestOptions = {}): Promise<ApiResponse<T>> {
    ...
    for (let attempt = 0; attempt < maxAttempts; attempt++) {
      ...
      try {
        ...
        if (error instanceof StanleyAuthError) throw error;
        if (error instanceof StanleyApiError && error.statusCode < 500 && error.statusCode !== 429) throw error;
        if (error instanceof StanleyTimeoutError && attempt === maxAttempts - 1) throw error;

        // Retry on 429, 5xx, network errors, timeouts
        if (error instanceof StanleyRateLimitError && error.retryAfterMs) {
          await sleep(error.retryAfterMs);
        }
      }
    }

    throw lastError ?? new StanleyNetworkError("Request failed after retries");
  }
  ...
  private async singleRequest<T>(
    ...
    try {
      ...
      if (error instanceof StanleyApiError || error instanceof StanleyAuthError || error instanceof StanleyRateLimitError) {
        throw error;
      }
      ...
      if (error instanceof Error) {
        if (error.name === "AbortError") {
          if (externalSignal?.aborted) {
            throw new StanleyNetworkError("Request aborted", error);
          }
          throw new StanleyTimeoutError(timeout, error);
        }
        // Network errors (connection refused, DNS, etc.)
        throw new StanleyNetworkError(error.message, error);
      }
      throw new StanleyNetworkError(String(error));
    } finally {
  ...
  private async parseResponse<T>(response: Response): Promise<ApiResponse<T>> {
    if (response.status === 429) {
      ...
      throw new StanleyRateLimitError(retryAfter ? parseInt(retryAfter) * 1000 : undefined);
    }

    if (response.status === 401 || response.status === 403) {
      const text = await response.text().catch(() => "");
      throw new StanleyAuthError(text || `HTTP ${response.status}`, response.status);
    }

    if (!response.ok) {
      const text = await response.text().catch(() => "");
      throw new StanleyApiError(`HTTP ${response.status}: ${text}`, response.status, text);
    }

    const data = await response.json();

    // Stanley API returns { success, data, error, timestamp }
