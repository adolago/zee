//! Filesystem-backed persistence for Stanley notes, theses, trades, events, people, and sectors.
...
impl StanleyRuntime {
    ...
    pub fn default_data_dir() -> PathBuf {
        if let Ok(path) = std::env::var("STANLEY_DATA_DIR") {
            return PathBuf::from(path);
        }

        if let Some(dirs) = directories::BaseDirs::new() {
            if let Some(state_dir) = dirs.state_dir() {
                return state_dir.join("zee/stanley");
            }
        }

        PathBuf::from(".stanley")
    }
    ...
    pub fn create_thesis(&self, request: CreateThesisRequest) -> Result<NoteResponse> {
        ...
        let document = Document {
            ...
            content: format!(
                "# {title}\n\n## Snapshot\n- Symbol: {symbol}\n- Company: {company_name}\n- Sector: {sector}\n- Conviction: {}\n\n## Thesis\nDeterministic Stanley runtime thesis scaffold for {symbol}.\n\n## Key Questions\n- What must happen for the thesis to strengthen?\n- What would invalidate the thesis?\n",
                request.conviction
            ),
            created_at: now,
            updated_at: now,
...
fn legacy_vault_path() -> PathBuf {
    if let Some(dirs) = directories::BaseDirs::new() {
        return dirs.home_dir().join(".stanley").join("vault");
    }
    PathBuf::from(".stanley/vault")
}
...
mod tests {
    ...
    fn runtime() -> StanleyRuntime {
        let dir = tempdir().unwrap();
        StanleyRuntime::new(dir.keep())
    }
