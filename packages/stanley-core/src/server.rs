//! Minimal HTTP runtime for Stanley.
...
use crate::runtime::{
    ...
    CreateThesisRequest, CreateTradeRequest, StanleyRuntime,
};
use crate::surface::*;
...
impl Default for ServerOptions {
    fn default() -> Self {
        let host = std::env::var("STANLEY_HOST").unwrap_or_else(|_| "127.0.0.1".to_string());
        let port = std::env::var("STANLEY_PORT")
            .ok()
        ...
        Self {
            host,
            port,
            data_dir: StanleyRuntime::default_data_dir(),
        }
    }
}
...
pub fn serve(options: ServerOptions) -> Result<()> {
    ...
    let state = Arc::new(ServerState {
        runtime: StanleyRuntime::new(options.data_dir),
        started_at: Instant::now(),
    });

    println!(
        "Stanley runtime listening on http://{}:{}",
    ...
    for stream in listener.incoming() {
        ...
        thread::spawn(move || {
            if let Err(error) = handle_connection(stream, state) {
                eprintln!("Stanley connection error: {error}");
            }
        });
    }
...
mod tests {
    ...
    fn state() -> ServerState {
        ...
        ServerState {
            runtime: StanleyRuntime::new(dir.keep()),
            started_at: Instant::now(),
        }
    }
    ...
    fn notes_endpoints_persist_documents() {
        ...
        assert_eq!(
            json["data"]["content"],
            Value::String("hello stanley".to_string())
        );
    }
