//! Stanley HTTP surface builders for market, portfolio, macro, and analytics routes.
...
pub fn build_recession_probability(country: &str) -> Value {
    ...
    json!({
        ...
        "modelVersion":"stanley-core-regime-v1",
...
fn macro_indicator_rows(country: &str) -> Vec<Value> {
    let snapshot = macro_snapshot(country);
    vec![
        json!({"category":"growth","code":"GDP_GROWTH","name":"GDP Growth","value": round2(snapshot.gdp_growth),"previousValue": round2(snapshot.gdp_growth - 0.2),"change":0.2,"unit":"%","frequency":"quarterly","lastUpdate":now(),"source":"stanley-core"}),
        json!({"category":"inflation","code":"CPI","name":"Inflation","value": round2(snapshot.inflation),"previousValue": round2(snapshot.inflation + 0.1),"change":-0.1,"unit":"%","frequency":"monthly","lastUpdate":now(),"source":"stanley-core"}),
        json!({"category":"labor","code":"UNEMPLOYMENT","name":"Unemployment","value": round2(snapshot.unemployment),"previousValue": round2(snapshot.unemployment + 0.1),"change":-0.1,"unit":"%","frequency":"monthly","lastUpdate":now(),"source":"stanley-core"}),
        json!({"category":"rates","code":"POLICY_RATE","name":"Policy Rate","value": round2(snapshot.policy_rate),"previousValue": round2(snapshot.policy_rate),"change":0.0,"unit":"%","frequency":"meeting","lastUpdate":now(),"source":"stanley-core"}),
        json!({"category":"external","code":"CURRENT_ACCOUNT","name":"Current Account","value": round2(snapshot.current_account),"previousValue": round2(snapshot.current_account - 0.2),"change":0.2,"unit":"% GDP","frequency":"quarterly","lastUpdate":now(),"source":"stanley-core"}),
    ]
}
