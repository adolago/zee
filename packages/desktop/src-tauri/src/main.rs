...
fn configure_display_backend() -> Option<String> {
    ...
    let allow_wayland = matches!(
        env::var("ZEE_ALLOW_WAYLAND").or_else(|_| env::var("OC_ALLOW_WAYLAND")),
        Ok(v) if matches!(v.to_ascii_lowercase().as_str(), "1" | "true" | "yes")
    );
    if allow_wayland {
        return Some("Wayland session detected; respecting ZEE_ALLOW_WAYLAND=1".into());
    }
    ...
    if env::var_os("DISPLAY").is_some() {
        ...
        return Some(
            "Wayland session detected; forcing X11 backend to avoid compositor protocol errors. \
               Set ZEE_ALLOW_WAYLAND=1 to keep native Wayland."
                .into(),
        );
    }
