...
- **macOS/Linux**: `~/.local/share/agent-core/log/`
- **Windows**: Press `WIN+R` and paste `%USERPROFILE%\.local\share\agent-core\log`

Log files are named with timestamps (e.g., `2025-01-09T123456.log`) and the most recent 10 log files are kept.

You can set the log level with the `--log-level` command-line option to get more detailed debug information. For example, `agent-core --log-level DEBUG`.
...
- **macOS/Linux**: `~/.local/share/agent-core/`
- **Windows**: Press `WIN+R` and paste `%USERPROFILE%\.local\share\agent-core`
...
OpenCode Desktop runs a local OpenCode server (the `agent-core-cli` sidecar) in the background. Most issues are caused by a misbehaving plugin, a corrupted cache, or a bad server setting.
...
- **macOS/Linux**: `~/.config/agent-core/agent-core.jsonc` (or `~/.config/agent-core/agent-core.json`)
- **macOS/Linux** (older installs): `~/.local/share/agent-core/agent-core.jsonc`
- **Windows**: Press `WIN+R` and paste `%USERPROFILE%\.config\agent-core\agent-core.jsonc`
...
- **Global plugins**
  - **macOS/Linux**: `~/.config/agent-core/plugins/`
  - **Windows**: Press `WIN+R` and paste `%USERPROFILE%\.config\agent-core\plugins`
- **Project plugins** (only if you use per-project config)
  - `<your-project>/.agent-core/plugins/`
...
- **macOS**: Finder -> `Cmd+Shift+G` -> paste `~/.cache/agent-core`
- **Linux**: delete `~/.cache/agent-core` (or run `rm -rf ~/.cache/agent-core`)
- **Windows**: Press `WIN+R` and paste `%USERPROFILE%\.cache\agent-core`
...
If your `agent-core.json(c)` contains a `server` section, temporarily remove it and restart the desktop app.

#### Check environment variables

If you have `AGENT_CORE_PORT` set in your environment, the desktop app will try to use that port for the local server.

- Unset `AGENT_CORE_PORT` (or pick a free port) and restart.
...
- `agent-core.settings.dat` (desktop default server URL)
- `agent-core.global.dat` and `agent-core.workspace.*.dat` (UI state like recent servers/projects)
...
3. Ensure you have the latest version with `agent-core upgrade`
...
To figure out what models you have access to, run `agent-core models`
...
2. If the issue persists, try clearing your stored configuration:
   ...
   ```bash
   rm -rf ~/.local/share/agent-core
   ```

   On Windows, press `WIN+R` and delete: `%USERPROFILE%\.local\share\agent-core`
...
1. Clear the provider package cache:
   ...
   ```bash
   rm -rf ~/.cache/agent-core
   ```

   On Windows, press `WIN+R` and delete: `%USERPROFILE%\.cache\agent-core`
