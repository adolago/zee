...
OpenCode Desktop runs a local OpenCode server (the `zee-cli` sidecar) in the background. Most issues are caused by a misbehaving plugin, a corrupted cache, or a bad server setting.
