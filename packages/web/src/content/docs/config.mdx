...
- `busy_submit_behavior` - What happens when you submit a prompt while the session is already running: `"followup"` queues the message, `"steer"` interrupts and immediately starts a new run, `"reject"` refuses to submit.
