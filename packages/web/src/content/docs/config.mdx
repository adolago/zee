...
1. **Remote config** (from `.well-known/agent-core`) - organizational defaults
2. **Global config** (`~/.config/agent-core/agent-core.json`) - user preferences
3. **Custom config** (`AGENT_CORE_CONFIG` env var) - custom overrides
4. **Project config** (`agent-core.json` in project) - project-specific settings
5. **`.agent-core` directories** - agents, commands, plugins
6. **Inline config** (`AGENT_CORE_CONFIG_CONTENT` env var) - runtime overrides

This means project configs can override global defaults, and global configs can override remote organizational defaults.

:::note
The `.agent-core` and `~/.config/agent-core` directories use **plural names** for subdirectories: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/`, and `themes/`. Singular names (e.g., `agent/`) are also supported for backwards compatibility.
:::
...
Organizations can provide default configuration via the `.well-known/agent-core` endpoint. This is fetched automatically when you authenticate with a provider that supports it.
...
```json title="Remote config from .well-known/agent-core"
{
  "mcp": {
    "jira": {
      "type": "remote",
...
Place your global OpenCode config in `~/.config/agent-core/agent-core.json`. Use global config for user-wide preferences like themes, providers, or keybinds.
...
Add `agent-core.json` in your project root. Project config has the highest precedence among standard config files - it overrides both global and remote configs.
...
Specify a custom config file path using the `AGENT_CORE_CONFIG` environment variable.

```bash
export AGENT_CORE_CONFIG=/path/to/my/custom-config.json
...
Specify a custom config directory using the `AGENT_CORE_CONFIG_DIR`
environment variable. This directory will be searched for agents, commands,
modes, and plugins just like the standard `.agent-core` directory, and should
follow the same structure.

```bash
export AGENT_CORE_CONFIG_DIR=/path/to/my/config-directory
agent-core run "Hello world"
```

The custom directory is loaded after the global config and `.agent-core` directories, so it **can override** their settings.
...
You can configure server settings for the `agent-core serve` and `agent-core web` commands through the `server` option.
...
You can also define agents using markdown files in `~/.config/agent-core/agents/` or `.agent-core/agents/`. [Learn more here](/docs/agents).
...
This setting applies across all interfaces: TUI, CLI (`agent-core run`), desktop app, and GitHub Action.
...
You can also define commands using markdown files in `~/.config/agent-core/commands/` or `.agent-core/commands/`. [Learn more here](/docs/commands).
...
Place plugin files in `.agent-core/plugins/` or `~/.config/agent-core/plugins/`. You can also load plugins from npm through the `plugin` option.
