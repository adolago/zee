...
2. **User config directory** - Defined in `~/.config/zee/themes/*.json` or `$XDG_CONFIG_HOME/agent-core/themes/*.json`
3. **Project root directory** - Defined in the `<project-root>/.zee/themes/*.json`
4. **Current working directory** - Defined in `./.zee/themes/*.json`
...
```bash no-frame
mkdir -p ~/.config/zee/themes
vim ~/.config/zee/themes/my-theme.json
```
