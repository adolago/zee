...
2. **User config directory** - Defined in `~/.config/agent-core/themes/*.json` or `$XDG_CONFIG_HOME/agent-core/themes/*.json`
3. **Project root directory** - Defined in the `<project-root>/.agent-core/themes/*.json`
4. **Current working directory** - Defined in `./.agent-core/themes/*.json`
...
```bash no-frame
mkdir -p ~/.config/agent-core/themes
vim ~/.config/agent-core/themes/my-theme.json
```
...
vim .agent-core/themes/my-theme.json
```
