...
- `.agent-core/plugins/` - Project-level plugins
- `~/.config/agent-core/plugins/` - Global plugins
...
**npm plugins** are installed automatically using Bun at startup. Packages and their dependencies are cached in `~/.cache/agent-core/node_modules/`.
...
1. Global config (`~/.config/agent-core/agent-core.json`)
2. Project config (`opencode.json`)
3. Global plugin directory (`~/.config/agent-core/plugins/`)
4. Project plugin directory (`.agent-core/plugins/`)
...
```json title=".agent-core/package.json"
{
  "dependencies": {
    "shescape": "^2.1.0"
  }
...
```ts title=".agent-core/plugins/my-plugin.ts"
...
```js title=".agent-core/plugins/example.js"
...
import type { Plugin } from "@agent-core/plugin"
...
```js title=".agent-core/plugins/notification.js"
...
```javascript title=".agent-core/plugins/env-protection.js"
...
```ts title=".agent-core/plugins/custom-tools.ts"
import { type Plugin, tool } from "@agent-core/plugin"
...
```ts title=".agent-core/plugins/compaction.ts"
import type { Plugin } from "@agent-core/plugin"
...
```ts title=".agent-core/plugins/custom-compaction.ts"
import type { Plugin } from "@agent-core/plugin"
