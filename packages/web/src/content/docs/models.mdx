...
Most popular providers are preloaded by default. If you've added the credentials for a provider through the `/connect` command, they'll be available when you start Zee.
...
Here are several models that work well with Zee, in no particular order. (This is not an exhaustive list nor is it necessarily up to date):
...
```jsonc title="zee.jsonc" {7-12,19-24}
{
  "$schema": "https://zee-bot.com/config.json",
  "provider": {
    "openai": {
...
Many models support multiple variants with different configurations. Zee ships with built-in default variants for popular providers.

### Built-in variants

Zee ships with default variants for many providers:
...
When Zee starts up, it checks for models in the following priority order:

1. The `--model` or `-m` command line flag. The format is the same as in the config file: `provider_id/model_id`.

2. The model list in the Zee config.
