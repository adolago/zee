...
- **Using Homebrew on macOS and Linux**
  ...
  ```bash
  brew install anomalyco/tap/agent-core
  ```

  > We recommend using the OpenCode tap for the most up to date releases. The official `brew install agent-core` formula is maintained by the Homebrew team and is updated less frequently.

- **Using Paru on Arch Linux**

  ```bash
  paru -S agent-core-bin
  ```
...
- **Using Chocolatey**
  ...
  ```bash
  choco install agent-core
  ```

- **Using Scoop**

  ```bash
  scoop install agent-core
  ```
...
- **Using Mise**
  ...
  ```bash
  mise use -g github:anomalyco/agent-core
  ```

- **Using Docker**

  ```bash
  docker run -it --rm ghcr.io/anomalyco/agent-core
  ```
