---
title: Server
description: Interact with zee server over HTTP.
---

import config from "../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

The `zee serve` command runs a headless HTTP server that exposes an OpenAPI endpoint that a zee client can use.
...
```bash
zee serve [--port <number>] [--hostname <string>] [--cors <origin>]
```
...
```bash
zee serve --cors http://localhost:5173 --cors https://app.example.com
```
...
Set `ZEE_SERVER_PASSWORD` to protect the server with HTTP basic auth. The username defaults to `zee`, or set `ZEE_SERVER_USERNAME` to override it. This applies to both `zee serve` and `zee web`.

```bash
ZEE_SERVER_PASSWORD=your-password zee serve
```
...
When you run `zee` it starts a TUI and a server. Where the TUI is the
...
:::tip
Use the zee server to interact with zee programmatically.
:::

This architecture lets zee support multiple clients and allows you to interact with zee programmatically.

You can run `zee serve` to start a standalone server. If you have the
zee TUI running, `zee serve` will start a new server.
...
The [`/tui`](#tui) endpoint can be used to drive the TUI through the server. For example, you can prefill or run a prompt. This setup is used by the Zee [IDE](/docs/ide) plugins.
...
The zee server exposes the following APIs.
