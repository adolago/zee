...
The `zee serve` command runs a headless HTTP server that exposes an OpenAPI endpoint that an opencode client can use.
...
opencode TUI running, `zee serve` will start a new server.
