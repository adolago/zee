...
Set `AGENT_CORE_SERVER_PASSWORD` to protect the server with HTTP basic auth. The username defaults to `opencode`, or set `AGENT_CORE_SERVER_USERNAME` to override it. This applies to both `opencode serve` and `opencode web`.

```bash
AGENT_CORE_SERVER_PASSWORD=your-password opencode serve
```
