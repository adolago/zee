...
Zee has a list of keybinds that you can customize through the Zee config.
...
Zee uses a `leader` key for most keybinds. This avoids conflicts in your terminal.
...
The Zee desktop app prompt input supports common Readline/Emacs-style shortcuts for editing text. These are built-in and currently not configurable via `zee.json`.
