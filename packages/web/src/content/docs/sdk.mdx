...
const { client } = await createAgentCore()
```
...
You can pass a configuration object to customize behavior. The instance still picks up your `agent-core.json`, but you can override or add configuration inline:

```javascript
import { createAgentCore } from "@agent-core/sdk"

const opencode = await createAgentCore({
  hostname: "127.0.0.1",
  port: 4096,
  config: {
...
```typescript
import type { Session, Message, Part } from "@agent-core/sdk"
```
