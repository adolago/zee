---
title: SDK
description: Type-safe JS client for Zee server.
---

import config from "../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

The Zee JS/TS SDK provides a type-safe client for interacting with Zee.
Use it to build integrations and control Zee programmatically.
...
Create an instance of Zee:

```javascript
import { createZee } from "@zee/sdk"

const { client } = await createZee()
```
...
You can pass a configuration object to customize behavior. The instance still picks up your `zee.json`, but you can override or add configuration inline:

```javascript
import { createZee } from "@zee/sdk"

const zee = await createZee({
  hostname: "127.0.0.1",
  port: 4096,
  config: {
...
console.log(`Server running at ${zee.server.url}`)
...
If you already have a running instance of Zee, you can create a client instance to connect to it:
...
```typescript
import type { Session, Message, Part } from "@zee/sdk"
```
