...
You can add external tools to Zee using the _Model Context Protocol_, or MCP. Zee supports both local and remote servers.
...
You can define MCP servers in your [Zee Config](https://zee-bot.com/docs/config/) under `mcp`. Add each MCP with a unique name. You can refer to that MCP by name when prompting the LLM.

```jsonc title="zee.jsonc" {6}
{
...
Organizations can provide default MCP servers via their `.well-known/zee` endpoint. These servers may be disabled by default, allowing users to opt-in to the ones they need.
...
Zee automatically handles OAuth authentication for remote MCP servers. When a server requires authentication, Zee will:
...
If the server requires authentication, Zee will prompt you to authenticate when you first try to use it. If not, you can [manually trigger the flow](#authenticating) with `zee mcp auth <server-name>`.
...
```bash
zee mcp logout my-oauth-server
```

The `mcp auth` command will open your browser for authorization. After you authorize, Zee will store the tokens securely in `~/.local/share/zee/mcp-auth.json`.
...
Your MCPs are available as tools in Zee, alongside built-in tools. So you can manage them through the Zee config like any other tool.
...
This will open a browser window to complete the OAuth flow and connect Zee to your Sentry account.
