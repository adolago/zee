---
title: Custom Tools
description: Create tools the LLM can call in zee.
---

Custom tools are functions you create that the LLM can call during conversations. They work alongside zee's [built-in tools](/docs/tools) like `read`, `write`, and `bash`.
...
- Locally by placing them in the `.zee/tools/` directory of your project.
- Or globally, by placing them in `~/.config/zee/tools/`.
...
```ts title=".zee/tools/database.ts" {1}
...
```ts title=".zee/tools/project.ts" {8}
import { tool } from "@zee/plugin"
...
```ts title=".zee/tools/python-add.ts" {10}
import { tool } from "@zee/plugin"
...
export default tool({
  ...
  async execute(args, context) {
    const script = path.join(context.worktree, ".zee/tools/add.py")
