...
- Locally by placing them in the `.agent-core/tools/` directory of your project.
- Or globally, by placing them in `~/.config/agent-core/tools/`.
...
```ts title=".agent-core/tools/database.ts" {1}
...
```ts title=".agent-core/tools/math.ts"
import { tool } from "@agent-core/plugin"
...
```ts title=".agent-core/tools/project.ts" {8}
import { tool } from "@agent-core/plugin"
...
```python title=".agent-core/tools/add.py"
import sys
...
```ts title=".agent-core/tools/python-add.ts" {10}
import { tool } from "@agent-core/plugin"
...
export default tool({
  ...
  async execute(args, context) {
    const script = path.join(context.worktree, ".agent-core/tools/add.py")
