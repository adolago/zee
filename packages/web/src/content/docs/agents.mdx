...
There are two types of agents in Zee; primary agents and subagents.
...
Zee comes with two built-in primary agents, **Build** and **Plan**. We'll
look at these below.
...
Zee comes with two built-in subagents, **General** and **Explore**. We'll look at this below.

---

## Built-in

Zee comes with two built-in primary agents and two built-in subagents.
...
Configure agents in your `zee.json` config file:
...
- Global: `~/.config/zee/agents/`
- Per-project: `.zee/agents/`

```markdown title="~/.config/zee/agents/review.md"
---
...
If no temperature is specified, Zee uses model-specific defaults; typically 0 for most models, 0.55 for Qwen models.
...
This path is relative to where the config file is located. So this works for both the global Zee config and the project specific config.
...
The model ID in your Zee config uses the format `provider/model-id`. For example, if you're using [Opencode Zen](/docs/zen), you would use `opencode/gpt-5.1-codex` for GPT 5.1 Codex.
...
:::tip
Run `zee models` to see a list of the available models.
:::
...
:::tip
Do you have an agent you'd like to share? [Submit a PR](https://github.com/adolago/zee).
:::
...
```markdown title="~/.config/zee/agents/docs-writer.md"
---
...
```markdown title="~/.config/zee/agents/security-auditor.md"
---
