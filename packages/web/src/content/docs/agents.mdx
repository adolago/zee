...
Configure agents in your `agent-core.json` config file:
...
- Global: `~/.config/agent-core/agents/`
- Per-project: `.agent-core/agents/`

```markdown title="~/.config/agent-core/agents/review.md"
---
...
The model ID in your OpenCode config uses the format `provider/model-id`. For example, if you're using [OpenCode Zen](/docs/zen), you would use `agent-core/gpt-5.1-codex` for GPT 5.1 Codex.
...
```json title="agent-core.json" {3-6,9-12}
{
  "$schema": "https://opencode.ai/config.json",
  "tools": {
    "write": true,
...
```json title="agent-core.json" {3-5,8-10}
{
  "$schema": "https://opencode.ai/config.json",
  "permission": {
    "edit": "deny"
...
```json title="agent-core.json" {6,7}
{
  "agent": {
    "deep-thinker": {
...
:::tip
Run `agent-core models` to see a list of the available models.
:::
...
```markdown title="~/.config/agent-core/agents/docs-writer.md"
---
...
```markdown title="~/.config/agent-core/agents/security-auditor.md"
---
