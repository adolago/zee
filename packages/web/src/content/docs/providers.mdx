---
title: Providers
description: Using any LLM provider in Zee.
---

import config from "../../../config.mjs"
export const console = config.console

Zee uses the [AI SDK](https://ai-sdk.dev/) and [Models.dev](https://models.dev) to support **75+ LLM providers** and it supports running local models.

To add a provider you need to:

1. Add the API keys for the provider using the `/connect` command.
2. Configure the provider in your Zee config.
...
in `~/.local/share/zee/auth.json`.

---

### Config

You can customize the providers through the `provider` section in your Zee
config.
...
Opencode Zen is a list of models provided by the Zee team that have been
tested and verified to work well with Zee. [Learn more](/docs/zen).

:::tip
If you are new, we recommend starting with Opencode Zen.
:::

1. Run the `/connect` command in the TUI, select opencode, and head to [zee-bot.com/auth](https://zee-bot.com/providers/opencode-zen).
...
It works like any other provider in Zee and is completely optional to use.
...
To use Amazon Bedrock with Zee:
...
2. **Configure authentication** using one of the following methods:
   ...
   Set one of these environment variables while running zee:

   ```bash
   # Option 1: Using AWS access keys
   AWS_ACCESS_KEY_ID=XXX AWS_SECRET_ACCESS_KEY=YYY zee

   # Option 2: Using named AWS profile
   AWS_PROFILE=my-profile zee

   # Option 3: Using Bedrock bearer token
   AWS_BEARER_TOKEN_BEDROCK=XXX zee
   ```
   ...
   For project-specific or persistent configuration, use `zee.json`:
...
:::info
Using your Claude Pro/Max subscription in Zee is not officially supported by [Anthropic](https://anthropic.com).
:::
...
2. Go to [Azure AI Foundry](https://ai.azure.com/) and deploy a model.
   ...
   :::note
   The deployment name must match the model name for zee to work properly.
   :::
...
5. Set your resource name as an environment variable:
   ...
   ```bash
   AZURE_COGNITIVE_SERVICES_RESOURCE_NAME=XXX zee
   ```
...
5. Run the `/models` command to select a model.
   ...
   You can also add models through your zee config.
...
to store token in zee auth storage.
:::

##### Self-Hosted GitLab

:::note[compliance note]
Zee uses a small model for some AI tasks like generating the session title.
It is configured to use gpt-5-nano by default, hosted by Zen. To lock Zee
to only use your own GitLab-hosted instance, add the following to your
`zee.json` file. It is also recommended to disable session sharing.
...
To use your GitHub Copilot subscription with zee:
...
To use Google Vertex AI with Zee:
...
2. Set the required environment variables:
   ...
   Set them while running zee.

   ```bash
   GOOGLE_APPLICATION_CREDENTIALS=/path/to/service-account.json GOOGLE_CLOUD_PROJECT=your-project-id zee
   ```
...
In the event you see a feature or model from Helicone that isn't configured automatically through zee, you can always configure it yourself.
...
```jsonc title="~/.config/zee/zee.jsonc"
{
  "$schema": "https://zee-bot.com/config.json",
  "provider": {
    "helicone": {
...
Helicone's [Sessions](https://docs.helicone.ai/features/sessions) feature lets you group related LLM requests together. Use the [opencode-helicone-session](https://github.com/H2Shami/opencode-helicone-session) plugin to automatically log each Zee conversation as a session in Helicone.
...
The plugin injects `Helicone-Session-Id` and `Helicone-Session-Name` headers into your requests. In Helicone's Sessions page, you'll see each Zee conversation listed as a separate session.
...
You can configure zee to use local models through [llama.cpp's](https://github.com/ggml-org/llama.cpp) llama-server utility

```json title="zee.json" "llama.cpp" {5, 6, 8, 10-15}
{
  "$schema": "https://zee-bot.com/config.json",
  "provider": {
    "llama.cpp": {
...
You can configure zee to use local models through LM Studio.

```json title="zee.json" "lmstudio" {5, 6, 8, 10-14}
{
  "$schema": "https://zee-bot.com/config.json",
  "provider": {
    "lmstudio": {
...
You can configure zee to use local models through Ollama.

:::tip
Ollama can automatically configure itself for Zee. See the [Ollama integration docs](https://docs.ollama.com/integrations/opencode) for details.
:::

```json title="zee.json" "ollama" {5, 6, 8, 10-14}
{
  "$schema": "https://zee-bot.com/config.json",
  "provider": {
    "ollama": {
...
You can configure Zee to use local or self-hosted vLLM through the OpenAI-compatible API.

1. Start a vLLM server exposing `/v1` endpoints (for example `http://localhost:8000/v1`).

2. Run `/connect` and select **vLLM (Local)**.

   ```txt
   /connect
   ```

3. Enter your base URL. Optionally enter an API key if your vLLM server requires auth.

4. Zee can probe `/models` to discover available model IDs.

5. Use `/models` to select one, or set one directly in config:
...
:::tip
If your server does not require authentication, leave the API key blank during setup.
:::
...
3. Copy the API key for use in Zee.
...
6. **Important**: Before using cloud models in Zee, you must pull the model information locally:
...
Opencode Zen is a list of tested and verified models provided by the Zee team. [Learn more](/docs/zen).
...
4. Many OpenRouter models are preloaded by default, run the `/models` command to select the one you want.
   ...
   You can also add additional models through your zee config.
...
5. You can also customize them through your zee config. Here's an example of specifying a provider
...
3. Enter your service key JSON.
   ...
   ```bash
   AICORE_SERVICE_KEY='{"clientid":"...","clientsecret":"...","url":"...","serviceurls":{"AI_API_URL":"..."}}' zee
   ```
...
4. Optionally set deployment ID and resource group:
   ...
   ```bash
   AICORE_DEPLOYMENT_ID=your-deployment-id AICORE_RESOURCE_GROUP=your-resource-group zee
   ```
...
To use [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) with Zee:
...
You can also customize models through your zee config. Here's an example of specifying provider routing order.
...
:::tip
You can use any OpenAI-compatible provider with zee. Most modern AI providers offer OpenAI-compatible APIs.
:::
...
3. Enter your API key for the provider.
   ...
   ┌  Add credential
   │
   ▲  This only stores a credential for myprovider - you will need to configure it in zee.json, check the docs for examples.
   │
   ◇  Enter your API key
   │  sk-...
   └
...
4. Create or update your `zee.json` file in your project directory:

   ```json title="zee.json" ""myprovider"" {5-15}
   {
     "$schema": "https://zee-bot.com/config.json",
     "provider": {
       "myprovider": {
...
```json title="zee.json" {9,11,17-20}
{
  "$schema": "https://zee-bot.com/config.json",
  "provider": {
    "myprovider": {
...
The `limit` fields allow Zee to understand how much context you have left. Standard providers pull these from models.dev automatically.
...
1. **Check the auth setup**: Run `zee auth list` to see if the credentials
...
2. For custom providers, check the zee config and:
   - Make sure the provider ID used in the `/connect` command matches the ID in your zee config.
