...
- `edit` - Cannot modify existing files, except for files located at `.agent-core/plans/*.md` to detail the plan itself
...
Configure modes in your `agent-core.json` config file:
...
- Global: `~/.config/agent-core/modes/`
- Project: `.agent-core/modes/`

```markdown title="~/.config/agent-core/modes/review.md"
---
model: anthropic/claude-sonnet-4-20250514
temperature: 0.1
tools:
...
```json title="agent-core.json" {4-14}
{
  "$schema": "https://opencode.ai/config.json",
  "mode": {
    "docs": {
...
Create mode files in `.agent-core/modes/` for project-specific modes or `~/.config/agent-core/modes/` for global modes:

```markdown title=".agent-core/modes/debug.md"
---
temperature: 0.1
tools:
  bash: true
...
```markdown title="~/.config/agent-core/modes/refactor.md"
---
model: anthropic/claude-sonnet-4-20250514
temperature: 0.2
tools:
