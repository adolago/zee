...
:::caution
Modes are now configured through the `agent` option in the zee config. The
`mode` option is now deprecated. [Learn more](/docs/agents).
:::

Modes in zee allow you to customize the behavior, tools, and prompts for different use cases.

It comes with two built-in modes: **build** and **plan**. You can customize
these or configure your own through the zee config.
...
Zee comes with two built-in modes.
...
- `edit` - Cannot modify existing files, except for files located at `.zee/plans/*.md` to detail the plan itself
...
Configure modes in your `zee.json` config file:
...
- Global: `~/.config/zee/modes/`
- Project: `.zee/modes/`

```markdown title="~/.config/zee/modes/review.md"
---
model: anthropic/claude-sonnet-4-20250514
temperature: 0.1
tools:
...
both the global zee config and the project specific config.
...
Create mode files in `.zee/modes/` for project-specific modes or `~/.config/zee/modes/` for global modes:

```markdown title=".zee/modes/debug.md"
---
temperature: 0.1
tools:
  bash: true
...
```markdown title="~/.config/zee/modes/refactor.md"
---
model: anthropic/claude-sonnet-4-20250514
temperature: 0.2
tools:
