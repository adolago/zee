...
Goal: Zee Gateway running on an exe.dev VM, reachable from your laptop via: `https://<vm-name>.exe.xyz`

This page assumes exe.dev's default **exeuntu** image. If you picked a different distro, map packages accordingly.

## Beginner quick path

1) [https://exe.new/zee](https://exe.new/zee)
...
Tip: keep this VM **stateful**. Zee stores state under `~/.zee/` and `~/zee/`.
...
Run the Zee install script:

```bash
curl -fsSL https://zee-bot.com/install.sh | bash
```

## 4) Setup nginx to proxy Zee to port 8000
...
## 5) Access Zee and grant privileges

Access `https://<vm-name>.exe.xyz/?token=YOUR-TOKEN-FROM-TERMINAL`. Approve
devices with `zee devices list` and `zee device approve`. When in doubt,
