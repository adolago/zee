---
summary: "Zee on DigitalOcean (simple paid VPS option)"
read_when:
  - Setting up Zee on DigitalOcean
  - Looking for cheap VPS hosting for Zee
---
...
Run a persistent Zee Gateway on DigitalOcean for **$6/month** (or $4/mo with reserved pricing).
...
# View logs
journalctl --user -u zee-gateway.service -f
```
...
```bash
zee config set gateway.bind tailnet
zee gateway restart
```
...
- `~/zee/` — workspace (SOUL.md, memory, etc.)
...
journalctl -u zee --no-pager -n 50
```
