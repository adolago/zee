---
summary: "Zee on Raspberry Pi (budget self-hosted setup)"
read_when:
  - Setting up Zee on a Raspberry Pi
  - Running Zee on ARM devices
...
Run a persistent, always-on Zee Gateway on a Raspberry Pi for **~$35-80** one-time cost (no monthly fees).
...
Most Zee features work on ARM64, but some external binaries may need ARM builds:
...
```bash
# Check logs
journalctl -u zee --no-pager -n 100

# Common fix: rebuild
cd ~/zee  # if using hackable install
npm run build
sudo systemctl restart zee
```
