---
summary: "Model providers (LLMs) supported by Zee"
read_when:
...
Zee can use many LLM providers. Pick one, authenticate, then set the default
...
1) Authenticate with the provider (usually via `zee onboard`).
