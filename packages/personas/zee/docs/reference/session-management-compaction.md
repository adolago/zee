...
This document explains how Zee manages sessions end-to-end:
...
Zee persists sessions in two layers:
...
- Room/channel (group chats): `agent:<agentId>:<channel>:channel:<id>` or `...:room:<id>`
- Cron: `cron:<job.id>`
...
- **Manual mode** (`session.reset.mode: "manual"`) disables automatic daily/idle resets; only explicit resets create a new `sessionId`.
...
Zee intentionally does **not** “fix up” transcripts; the Gateway uses `SessionManager` to read/write them.
...
These are Pi runtime semantics (Zee consumes the events, but Pi decides when to compact).
...
Zee also enforces a safety floor for embedded runs:

- If `compaction.reserveTokens < reserveTokensFloor`, Zee bumps it.
- Default floor is `20000` tokens.
- Set `agents.defaults.compaction.reserveTokensFloor: 0` to disable the floor.
- If it’s already higher, Zee leaves it alone.
...
Zee supports “silent” turns for background tasks where the user should not see intermediate output.

Convention:
- The assistant starts its output with `NO_REPLY` to indicate “do not deliver a reply to the user”.
- Zee strips/suppresses this in the delivery layer.

As of `2026.1.10`, Zee also suppresses **draft/typing streaming** when a partial chunk begins with `NO_REPLY`, so silent operations don’t leak partial output mid-turn.
...
Zee uses the **pre-threshold flush** approach:
...
Pi also exposes a `session_before_compact` hook in the extension API, but Zee’s
...
- Store vs transcript mismatch? Confirm the Gateway host and the store path from `zee status`.
