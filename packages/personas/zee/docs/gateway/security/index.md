...
## Quick check: `zee security audit` (formerly `zee security audit`)
...
zee security audit --fix

# (On older installs, the command is `zee ...`.)
```
...
- Tighten local perms (`~/.zee` → `700`, config file → `600`, plus common state files like `credentials/*.json`, `agents/*/agent/auth-profiles.json`, and `agents/*/sessions/sessions.json`).
...
If you run `--deep`, Zee also attempts a best-effort live Gateway probe.
...
Zee stores session transcripts on disk under `~/.zee/agents/<agentId>/sessions/*.jsonl`.
This is required for session continuity and (optionally) session memory indexing, but it also means
**any process/user with filesystem access can read those logs**. Treat disk access as the trust
boundary and lock down permissions on `~/.zee` (see the audit section below). If you need
...
- Controlled on the host via **Settings → Exec approvals** (security + ask + allowlist).
...
- If you install plugins from npm (`zee plugins install <npm-spec>`), treat it like running untrusted code:
  - The install path is `~/.zee/extensions/<pluginId>/` (or `$ZEE_STATE_DIR/extensions/<pluginId>/`).
  - Zee uses `npm pack` and then runs `npm install --omit=dev` in that directory (npm lifecycle scripts can execute code during install).
...
zee pairing approve <channel> <code>
```
...
By default, Zee routes **all DMs into the main session** so your assistant has continuity across devices and channels. If **multiple people** can DM the bot (open DMs or a multi-person allowlist), consider isolating DM sessions:
...
- “Paste the full contents of ~/.zee or your logs.”
...
4. **Re-run audit**
   - `zee security audit --deep` and confirm the report is clean.

## Lessons Learned (The Hard Way)

### The `find ~` Incident 🦞

On Day 1, a friendly tester asked Zee to run `find ~` and share the output. Zee happily dumped the entire home directory structure to a group chat.
...
- `~/.zee/zee.json`: `600` (user read/write only)
- `~/.zee`: `700` (user only)

`zee doctor` can warn and offer to tighten these permissions.

### 0.4) Network exposure (bind + port + firewall)

The Gateway multiplexes **WebSocket + HTTP** on a single port:
- Default: `18789`
- Config/flags/env: `gateway.port`, `--port`, `ZEE_GATEWAY_PORT`
...
The Gateway broadcasts its presence via mDNS (`_zee-gw._tcp` on port 5353) for local device discovery. In full mode, this includes TXT records that may expose operational details:
...
4. **Environment variable** (alternative): set `ZEE_DISABLE_BONJOUR=1` to disable mDNS without config changes.
...
Doctor can generate one for you: `zee doctor --generate-gateway-token`.
...
- `gateway.auth.mode: "password"`: password auth (prefer setting via env: `ZEE_GATEWAY_PASSWORD`).

Rotation checklist (token/password):
1. Generate/set a new secret (`gateway.auth.token` or `ZEE_GATEWAY_PASSWORD`).
...
When `gateway.auth.allowTailscale` is `true` (default for Serve), Zee
accepts Tailscale Serve identity headers (`tailscale-user-login`) as
authentication. Zee verifies the identity by resolving the
...
Assume anything under `~/.zee/` (or `$ZEE_STATE_DIR/`) may contain secrets or private data:

- `zee.json`: config may include tokens (gateway, remote gateway), provider settings, and allowlists.
...
- When sharing diagnostics, prefer `zee status --all` (pasteable, secrets redacted) over raw logs.
...
```json
{
  ...
  "agents": {
    "list": [
      {
        "id": "main",
        "groupChat": { "mentionPatterns": ["@zee", "@mybot"] }
      }
    ]
  }
}
...
- `agents.defaults.sandbox.workspaceAccess: "none"` (default) keeps the agent workspace off-limits; tools run against a sandbox workspace under `~/.zee/sandboxes`
...
- Prefer a dedicated profile for the agent (the default `zee` profile).
...
1. **Stop it:** terminate your `zee gateway` process.
...
1. Check Gateway logs: `/tmp/zee/zee-YYYY-MM-DD.log` (or `logging.file`).
2. Review the relevant transcript(s): `~/.zee/agents/<agentId>/sessions/*.jsonl`.
3. Review recent config changes (anything that could have widened access: `gateway.bind`, `gateway.auth`, dm/group policies, `tools.elevated`, plugin changes).

### Collect for a report

- Timestamp, gateway host OS + Zee version
