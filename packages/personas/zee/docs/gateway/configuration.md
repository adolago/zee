...
Zee only accepts configurations that fully match the schema.
Unknown keys, malformed types, or invalid values cause the Gateway to **refuse to start** for safety.

When validation fails:
- The Gateway does not boot.
- Only diagnostic commands are allowed (for example: `zee doctor`, `zee logs`, `zee health`, `zee status`, `zee service`, `zee help`).
- Run `zee doctor` to see the exact issues.
- Run `zee doctor --fix` (or `--yes`) to apply migrations/repairs.
...
use `config.patch` or `zee config set`. Keep a backup of `~/.zee/zee.json`.
...
{
  agents: {
    defaults: { workspace: "~/zee" },
    list: [
      {
        id: "main",
        groupChat: { mentionPatterns: ["@zee", "reisponde"] }
      }
    ]
  },
  channels: {
...
{ "$include": "/etc/zee/base.json5" } // absolute
...
```json5
// ~/.zee/clients/mueller/agents.json5
[
...
```json5
// ~/.zee/clients/mueller/broadcast.json5
{
...
```json5
{
  ...
  agents: {
    list: [
      { id: "main", groupChat: { mentionPatterns: ["@zee", "zee", "zee"] } }
    ]
  }
}
```
...
- `agents.list[]`: per-agent overrides.
  ...
  - `workspace`: default `~/zee-<agentId>` (for `main`, falls back to `agents.defaults.workspace`).
...
```json5
{
  agents: {
    list: [
      { id: "home", default: true, workspace: "~/zee-home" },
      { id: "work", workspace: "~/zee-work" }
    ]
  },
  bindings: [
...
```json5
{
  agents: { defaults: { repoRoot: "~/Projects/zee" } }
}
```
...
When a file exceeds this limit, Zee logs a warning and injects a truncated
...
`[zee]`, so same-phone conversations stay legible.
...
`messages.messagePrefix`). Default stays **unchanged**: `"[zee]"` when
...
```json5
{
  messages: {
    tts: {
      ...
      maxTextLength: 4000,
      timeoutMs: 30000,
      prefsPath: "~/.zee/settings/tts.json",
      elevenlabs: {
...
- `tools.web.search.apiKey` (recommended: set via `zee configure --section web`, or use `BRAVE_API_KEY` env var)
...
Notes:
- Set `MOONSHOT_API_KEY` in the environment or use `zee onboard --auth-choice moonshot-api-key`.
...
Notes:
- Set `KIMICODE_API_KEY` in the environment or use `zee onboard --auth-choice kimi-code-api-key`.
...
Notes:
- Set `SYNTHETIC_API_KEY` or use `zee onboard --auth-choice synthetic-api-key`.
...
from `~/.zee/extensions`, `<workspace>/.zee/extensions`, plus any
...
### `browser` (zee-managed browser)

Zee can start a **dedicated, isolated** Chrome/Brave/Edge/Chromium instance for zee and expose a small loopback control service.
...
- When a connection comes from one of these IPs, Zee uses `x-forwarded-for` (or `x-real-ip`) to determine the client IP for local pairing checks and HTTP auth/local checks.
...
```json5
{
  bridge: {
    ...
    tls: {
      enabled: true,
      // Uses ~/.zee/bridge/tls/bridge-{cert,key}.pem when omitted.
      // certPath: "~/.zee/bridge/tls/bridge-cert.pem",
      // keyPath: "~/.zee/bridge/tls/bridge-key.pem"
    }
  }
}
```
...
Controls LAN mDNS discovery broadcasts (`_zee-gw._tcp`).
