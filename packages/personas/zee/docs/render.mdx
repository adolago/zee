...
Deploy Zee on Render using Infrastructure as Code. The included `render.yaml` Blueprint defines your entire stack declaratively, service, disk, environment variables, so you can deploy with a single click and version your infrastructure alongside your code.
...
<a href="https://render.com/deploy?repo=https://github.com/zee/zee" target="_blank" rel="noreferrer">Deploy to Render</a>
...
repository configures everything needed to run Zee:
...
services:
  - type: web
    ...
    envVars:
      ...
      - key: ZEE_STATE_DIR
        value: /data/.zee
      - key: ZEE_WORKSPACE_DIR
        value: /data/workspace
      - key: ZEE_GATEWAY_TOKEN
...
4. Optionally configure messaging channels (WhatsApp, WhatsApp)
5. Click **Run setup**
...
If you use the original Zee repository, Render will not auto-deploy your Zee. To update it, run a manual Blueprint sync from the dashboard.
...
For Zee, vertical scaling is usually sufficient. Horizontal scaling requires sticky sessions or external state management.
...
This downloads a portable backup you can restore on any Zee host.
