...
2) Configure WhatsApp in `~/.zee/zee.json`.
3) Run `zee channels login` to scan the QR code (Linked Devices).
4) Start the gateway.
...
- **CLI / CLI/TUI** talk to the gateway; no direct Baileys use.
...
Quick fallback: run Zee on **your own number**. Message yourself (WhatsApp “Message yourself”) for testing so you don’t spam contacts. Expect to read verification codes on your main phone during setup and experiments. **Must enable self-chat mode.**
...
Self-chat replies default to `[{identity.name}]` when set (otherwise `[zee]`)
...
## Login + credentials
- Login command: `zee channels login` (QR via Linked Devices).
- Multi-account login: `zee channels login --account <id>` (`<id>` = `accountId`).
...
- Logout: `zee channels logout` (or `--account <id>`) deletes WhatsApp auth state (but keeps shared `oauth.json`).
...
- **DM policy**: `channels.whatsapp.dmPolicy` controls direct chat access (default: `pairing`).
  - Pairing: unknown senders get a pairing code (approve via `zee pairing approve whatsapp <code>`; codes expire after 1 hour).
...
If you run Zee on your **personal WhatsApp number**, enable `channels.whatsapp.selfChatMode` (see sample above).
...
**Will Zee message random contacts when I link WhatsApp?**  
No. Default DM policy is **pairing**, so unknown senders only get a pairing code and their message is **not processed**. Zee only replies to chats it receives, or to sends you explicitly trigger (agent/CLI).

**How does pairing work on WhatsApp?**  
Pairing is a DM gate for unknown senders:
- First DM from a new sender returns a short code (message is not processed).
- Approve with: `zee pairing approve whatsapp <code>` (list with `zee pairing list whatsapp`).
- Codes expire after 1 hour; pending requests are capped at 3 per channel.

**Can multiple people use different Zees on one WhatsApp number?**  
...
- Media:
  ...
  - Animated GIFs: WhatsApp expects MP4 with `gifPlayback: true` for inline looping.
    - CLI: `zee message send --media <mp4> --gif-playback`
    - Gateway: `send` params include `gifPlayback: true`

## Voice notes (PTT audio)
WhatsApp sends audio as **voice notes** (PTT bubble).
- Best results: OGG/Opus. Zee rewrites `audio/ogg` to `audio/ogg; codecs=opus`.
...
- Fix: run `zee channels login` on the gateway host and scan the QR (WhatsApp → Settings → Linked Devices).
