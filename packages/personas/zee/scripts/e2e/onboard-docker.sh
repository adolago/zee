...
TRASH
  ...
  run_wizard_cmd() {
    ...
    input_fifo="$(mktemp -u "/tmp/zee-onboard-${case_name}.XXXXXX")"
    mkfifo "$input_fifo"
    local log_path="/tmp/zee-onboard-${case_name}.log"
  ...
  run_case_local_basic() {
    ...
    workspace_dir="$HOME/zee"
    config_path="$HOME/.zee/zee.json"
    sessions_dir="$HOME/.zee/agents/main/sessions"
