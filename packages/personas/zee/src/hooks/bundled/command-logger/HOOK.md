...
2. **Appends to log file** - Writes a JSON line to `~/.zee/logs/commands.log`
...
1. **Manual rotation**:
   ...
   ```bash
   mv ~/.zee/logs/commands.log ~/.zee/logs/commands.log.old
   ```

2. **Use logrotate** (Linux):
   Create `/etc/logrotate.d/zee`:
   ```
   /home/username/.zee/logs/commands.log {
       weekly
       rotate 4
       compress
       missingok
