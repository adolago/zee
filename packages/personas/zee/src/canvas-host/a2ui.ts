 ...
 */
export function normalizeCanvasPath(urlPath: string) {
 ...
 */
export function extractCanvasPersona(url: string | URL) {
...
export async function handleA2uiHttpRequest(
  ...
  const basePath =
    url.pathname === A2UI_PATH || url.pathname.startsWith(`${A2UI_PATH}/`)
      ? A2UI_PATH
      : url.pathname === AGENT_A2UI_PATH || url.pathname.startsWith(`${AGENT_A2UI_PATH}/`)
        ? AGENT_A2UI_PATH
        : undefined;
  if (!basePath) {
    return false;
