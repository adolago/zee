...
async function invokeCanvas(
  opts: { node?: string; invokeTimeout?: string; json?: boolean },
  command: string,
  ...
  return (await callGatewayCli("node.invoke", opts, invokeParams)) as unknown;
}
    ...
    canvas
      ...
      .action(async (opts) => {
        await runNodesCommand("canvas snapshot", async () => {
          ...
          const res = typeof raw === "object" && raw !== null ? (raw as Record<string, unknown>) : {};
    ...
    canvas
      ...
      .action(async (jsArg, opts) => {
        await runNodesCommand("canvas eval", async () => {
          const js = (opts as { js?: string }).js ?? jsArg;
          ...
          if (
            ...
            defaultRuntime.log(String((payload as Record<string, unknown>).result));
          } else {
            const { ok } = getNodesTheme();
    ...
    a2ui
      ...
      .action(async (opts) => {
        await runNodesCommand("canvas a2ui push", async () => {
          ...
          if (version === "v0.9") {
            throw new Error(
              "Detected A2UI v0.9 JSONL (createSurface). Zee currently supports v0.8 only.",
            );
          }
          ...
          if (!opts.json) {
            ...
            defaultRuntime.log(
              ok(
                `canvas a2ui push ok (v0.8, ${messageCount} message${
                  messageCount === 1 ? "" : "s"
                })`,
              ),
            );
