...
function collectReferencedAgentIds(cfg: ZeeConfig): string[] {
...
export function formatDuplicateAgentDirError(dups: DuplicateAgentDir[]): string {
  const lines: string[] = [
    ...
    "If you want to share credentials, use the shared agent-core auth.json instead of sharing the entire agentDir.",
  ];
  return lines.join("\n");
}
