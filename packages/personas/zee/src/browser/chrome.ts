...
import { DEFAULT_ZEE_BROWSER_COLOR, DEFAULT_ZEE_BROWSER_PROFILE_NAME } from "./constants.js";
...
export function resolveZeeUserDataDir(profileName = DEFAULT_ZEE_BROWSER_PROFILE_NAME) {
...
export async function launchZeeChrome(
  ...
  const userDataDir = resolveZeeUserDataDir(profile.name);
  fs.mkdirSync(userDataDir, { recursive: true });

  const needsDecorate = !isProfileDecorated(
    userDataDir,
    profile.name,
    (profile.color ?? DEFAULT_ZEE_BROWSER_COLOR).toUpperCase(),
  );
  ...
  if (needsDecorate) {
    try {
      decorateZeeProfile(userDataDir, {
        name: profile.name,
        color: profile.color,
      });
      log.info(`🦞 zee browser profile decorated (${profile.color})`);
    } catch (err) {
      log.warn(`zee browser profile decoration failed: ${String(err)}`);
    }
  }
  ...
  try {
    ...
    log.warn(`zee browser clean-exit prefs failed: ${String(err)}`);
  }
  ...
  log.info(
    `🦞 zee browser started (${exe.kind}) profile "${profile.name}" on 127.0.0.1:${profile.cdpPort} (pid ${pid})`,
  );
...
export async function stopZeeChrome(running: RunningChrome, timeoutMs = 2500) {
