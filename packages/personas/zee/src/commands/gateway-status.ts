...
export async function gatewayStatusCommand(
    ...
    async () => {
      ...
      if (!sshTarget && opts.sshAuto) {
        ...
        const candidates = discovery
          ...
          // Validate candidates through parseSshTarget to reject malformed targets
          // (e.g. hostnames starting with '-' that could cause SSH option injection).
          .filter((x) => parseSshTarget(x) !== null);
  ...
  runtime.log(
    discovery.length > 0
      ? `Found ${discovery.length} gateway(s) via Bonjour (local. + zee.internal.)`
      : "Found 0 gateways via Bonjour (local. + zee.internal.)",
  );
