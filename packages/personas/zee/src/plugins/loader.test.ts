...
const prevBundledDir = process.env.ZEE_BUNDLED_PLUGINS_DIR;
const EMPTY_PLUGIN_SCHEMA = { type: "object", additionalProperties: false, properties: {} };

function makeTempDir() {
  const dir = path.join(os.tmpdir(), `zee-plugin-${randomUUID()}`);
...
afterEach(() => {
  ...
  if (prevBundledDir === undefined) {
    ...
    process.env.ZEE_BUNDLED_PLUGINS_DIR = prevBundledDir;
  }
});

describe("loadZeePlugins", () => {
  it("disables bundled plugins by default", () => {
    ...
    const enabledRegistry = loadZeePlugins({
      cache: false,
      config: {
        plugins: {
          allow: ["bundled"],
