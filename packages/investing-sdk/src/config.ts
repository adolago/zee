...
export interface DaemonConfig {
  /** Auto-start Investing API if not running (default: true) */
  autoStart: boolean;
  /** Auto-restart on crash (default: true) */
  autoRestart: boolean;
  /** Investing core executable path used for local daemon startup */
  coreBin?: string;
  /** Path to Investing repo (auto-detected if not set) */
  repoPath?: string;
...
export interface InvestingSDKConfig {
  /** Base URL for Investing API (default: "http://127.0.0.1:8000") */
  baseUrl: string;
  /** Daemon configuration */
  daemon: DaemonConfig;
  /** HTTP transport configuration */
...
export const DEFAULT_CONFIG: InvestingSDKConfig = {
...
export type PartialSDKConfig = Omit<Partial<InvestingSDKConfig>, "daemon" | "http" | "auth" | "cache" | "ws"> & {
...
export function resolveConfig(partial?: PartialSDKConfig): InvestingSDKConfig {
