...
export class WhatsAppHandler implements MessagingPlatformHandler {
  ...
  async connect(): Promise<void> {
    ...
    try {
      ...
      if (!baileysModule) {
        throw new Error("Baileys not available. Install with: npm install @whiskeysockets/baileys")
      }
      ...
      // Set up auth state
      const { state: authState, saveCreds } = await useMultiFileAuthState(`./.baileys/${this.config.sessionName}`)
  ...
  private checkMention(messageContent: any, isGroup: boolean): boolean {
    ...
    if (contextInfo?.mentionedJid?.length > 0) {
      ...
      if (botJid) {
        return contextInfo.mentionedJid.some((jid: string) => jid.startsWith(botJid.split(":")[0]))
      }
    }
 ...
 */
export function createWhatsAppHandler(config: WhatsAppConfig, qrCallback?: (qr: string) => void): WhatsAppHandler {
