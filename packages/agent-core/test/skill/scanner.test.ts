...
describe("scanSource", () => {
  test("detects child_process exec with string interpolation", () => {
    const source = `
...
`
  ...
  test("returns empty array for normal http client code (just a fetch GET)", () => {
    const source = `
...
describe("isScannable", () => {
  test("accepts .js, .ts, .mjs, .cjs, .tsx, .jsx files", () => {
  ...
  test("rejects non-code files (.md, .json, .png, .css)", () => {
...
describe("scanDirectory", () => {
  test("scans .js files in a directory tree", async () => {
  ...
  test("scans hidden entry files when explicitly included", async () => {
    const root = makeTmpDir()
    const hidden = path.join(root, ".hidden")
    fsSync.mkdirSync(hidden, { recursive: true })
...
describe("scanDirectoryWithSummary", () => {
  ...
  test("caps scanned file count with maxFiles", async () => {
    const root = makeTmpDir()
  ...
  test("skips files above maxFileBytes", async () => {
    const root = makeTmpDir()
  ...
  test("ignores missing included files", async () => {
    const root = makeTmpDir()
  ...
  test("prioritizes included entry files when maxFiles is reached", async () => {
    const root = makeTmpDir()
  ...
  test("throws when reading a scannable file fails", async () => {
    const root = makeTmpDir()
    ...
    const spy = mock(() => {})
    // Monkey-patch fs.readFile to throw EACCES for the target file
    const patchedReadFile = async (...args: Parameters<typeof fs.readFile>) => {
    ...
    ;(fs as any).readFile = patchedReadFile
