"""
Stanley Cache Manager
=====================

Redis-based caching layer for Stanley API endpoints.
...
Usage:
    from stanley.cache import cache_manager, cached
...
class CacheManager:
    ...
    async def initialize(self) -> bool:
        ...
        redis_url = os.environ.get("STANLEY_REDIS_URL") or os.environ.get("REDIS_URL")
    ...
    async def get(self, key: str) -> Optional[Any]:
        ...
        if self._use_redis:
            try:
                value = await self._redis_client.get(f"stanley:{key}")
                if value:
        ...
        if self._use_redis:
            try:
                await self._redis_client.setex(
                    f"stanley:{key}", ttl, json.dumps(value, default=str)
                )
                return True
            except Exception as e:
    ...
    async def delete(self, key: str) -> bool:
        ...
        if self._use_redis:
            try:
                await self._redis_client.delete(f"stanley:{key}")
                deleted = True
            except Exception as e:
            ...
            try:
                ...
                while True:
                    cursor, keys = await self._redis_client.scan(
                        cursor=cursor, match=f"stanley:{pattern}", count=100
                    )
                    if keys:
