...
try:
    ...
    raise ImportError(
        "PyJWT is required for Stanley authentication. "
...
class JWTSettings:
    ...
    issuer: str = field(default="stanley-api")
    audience: str = field(default="stanley-client")
    ...
    def __post_init__(self):
        ...
        try:
            from stanley.api.auth.config import get_auth_settings
        ...
        self.issuer = os.getenv("JWT_ISSUER", "stanley-api")
        self.audience = os.getenv("JWT_AUDIENCE", "stanley-client")

        # Validate
        if not self.secret_key:
            raise ValueError(
                "JWT secret key not configured. Set STANLEY_AUTH_JWT_SECRET_KEY "
