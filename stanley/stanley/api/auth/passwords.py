...
def _get_crypt_context(schemes=None, deprecated=None, bcrypt__rounds=None):
    """
    Get a CryptContext, using a mock only if STANLEY_DEV_MODE is explicitly set.
    ...
    dev_mode = _os.getenv("STANLEY_DEV_MODE", "").lower() in ("true", "1", "yes")
    if not dev_mode:
        raise ImportError(
            "passlib is required for production. Install with: pip install passlib[bcrypt]\n"
            "To use mock password hashing for development, set STANLEY_DEV_MODE=true"
        )

    logging.getLogger(__name__).warning(
        "STANLEY_DEV_MODE=true: Using INSECURE mock password hashing. "
