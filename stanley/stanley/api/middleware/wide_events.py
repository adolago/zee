"""
Wide event logging middleware for Stanley API.
...
from stanley.config.logging import (
...
def _resolve_settings() -> WideEventsSettings:
    base_dir = os.environ.get(
        "STANLEY_WIDE_EVENTS_DIR",
        os.path.join(os.path.expanduser("~"), ".stanley", "logs"),
    )
    today = datetime.now(timezone.utc).strftime("%Y-%m-%d")
    file_path = os.environ.get(
        "STANLEY_WIDE_EVENTS_FILE",
        os.path.join(base_dir, f"stanley-wide-{today}.jsonl"),
    )
    enabled = os.environ.get("STANLEY_WIDE_EVENTS_ENABLED", "1") != "0"
    sample_rate = float(os.environ.get("STANLEY_WIDE_EVENTS_SAMPLE_RATE", "0.02"))
    slow_ms = float(os.environ.get("STANLEY_WIDE_EVENTS_SLOW_MS", "2000"))
    payloads = os.environ.get("STANLEY_WIDE_EVENTS_PAYLOADS", "debug")
