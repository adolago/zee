...
from stanley.api.settings import router
...
def test_settings_leak_prevention():
    ...
    with patch("stanley.api.settings.settings_manager.load") as mock_load:
...
def test_save_settings_leak_prevention():
    with patch("stanley.api.settings.settings_manager.update") as mock_update:
