...
@pytest.mark.skipif(app is None, reason="stanley.api.main_new not available")
def test_system_metrics_unauthorized_access(client):
    ...
    """
    with patch("stanley.core.circuit_breaker.get_all_circuit_breaker_stats", return_value=[]), \
         patch("stanley.core.health.get_health_monitor") as mock_health:
...
def test_component_health_unauthorized_access(client):
    ...
    with patch("stanley.core.health.get_health_monitor") as mock_health:
        mock_health.return_value.get_system_health.return_value = Mock(components=[])
