...
3) Add the MCP server with the Zee CLI:
```bash
zee mcp add render --url https://mcp.render.com/mcp --bearer-token-env-var RENDER_API_KEY
```

4) Restart Zee, then retry `list_services()`.
