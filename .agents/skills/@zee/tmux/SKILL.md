...
## Orchestrating Coding Agents (Zee, Claude Code)

tmux excels at running multiple coding agents in parallel:

```bash
SOCKET="${TMPDIR:-/tmp}/zee-army.sock"
...
tmux -S "$SOCKET" send-keys -t agent-1 "cd /tmp/project1 && zee --yolo 'Fix bug X'" Enter
tmux -S "$SOCKET" send-keys -t agent-2 "cd /tmp/project2 && zee --yolo 'Fix bug Y'" Enter
...
- `pnpm install` first before running zee in fresh clones
- Check for shell prompt (`❯` or `$`) to detect completion
- Zee needs `--yolo` or `--full-auto` for non-interactive fixes
