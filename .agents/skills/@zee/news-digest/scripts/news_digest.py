...
Zee's WebSearch and LLM providers. No separate API keys required.

Architecture:
  Stanley -> Zee
  - WebSearch via Exa MCP (built into Zee)
  - LLM summarization via Zee providers
...
def find_zee_path() -> Optional[Path]:
    """Find Zee installation path."""
    # Check common locations
    paths = [
        Path.home() / "Repositories/zee",
        Path.home() / ".zee",
    ]
    for p in paths:
        if p.exists():
...
def get_zee_bin() -> str:
    ...
    result = subprocess.run(["which", "zee"], capture_output=True, text=True)
    if result.returncode == 0:
        return result.stdout.strip()

    # Check common locations
    paths = [
        Path.home() / ".local/bin/zee",
        Path.home() / ".bun/bin/zee",
    ]
    for p in paths:
        if p.exists():
            return str(p)
...
async def search_via_exa(query: str, num_results: int = 5) -> list[dict]:
    """
    Search using Zee's WebSearch tool via Exa MCP.
    Uses EXA_API_KEY from environment if available.
    """
    try:
        import httpx

        # Use Exa MCP endpoint directly (same as Zee's websearch tool)
        search_request = {
            "jsonrpc": "2.0",
            "id": 1,
            "method": "tools/call",
...
async def search_ticker_news(
    ...
    for category in categories:
        ...
        results = await search_via_exa(query, results_per_category)
...
<body style="max-width: 800px; margin: 0 auto; padding: 20px;">
    ...
    <p style="color: #9ca3af; font-size: 12px;">Generated by Stanley via Zee</p>
</body>
</html>"""
...
def main():
    parser = argparse.ArgumentParser(
        description="Generate news digests for portfolio holdings (uses Zee infrastructure)"
    )
    parser.add_argument(
        "--tickers", "-t",
        required=True,
    ...
    parser.add_argument(
        "--summarize", "-s",
        action="store_true",
        help="Summarize articles using Zee LLM"
    )
    parser.add_argument(
        "--format", "-f",
        default="markdown",
