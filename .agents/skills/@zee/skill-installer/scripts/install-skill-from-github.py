#!/usr/bin/env python3
"""Install a skill from a GitHub repo path into $ZEE_HOME/skills."""
...
def _zee_home() -> str:
    return os.environ.get("ZEE_HOME", os.path.expanduser("~/.config/zee"))


def _tmp_root() -> str:
    base = os.path.join(tempfile.gettempdir(), "zee")
    os.makedirs(base, exist_ok=True)
    return base


def _request(url: str) -> bytes:
    return github_request(url, "zee-skill-install")
...
def _default_dest() -> str:
    return os.path.join(_zee_home(), "skills")
