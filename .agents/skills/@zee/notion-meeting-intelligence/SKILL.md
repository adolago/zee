...
description: Prepare meeting materials with Notion context and assistant research; use when gathering context, drafting agendas/pre-reads, and tailoring materials to attendees.
metadata:
...
Prep meetings by pulling Notion context, tailoring agendas/pre-reads, and enriching with assistant research.
...
5) Enrich with assistant research (industry insights, benchmarks, risks) and update the page with `Notion:notion-update-page` as plans change.

## Workflow
### 0) If any MCP call fails because Notion MCP is not connected, pause and set it up:
1. Add the Notion MCP:
   - `zee mcp add notion --url https://mcp.notion.com/mcp`
2. Enable remote MCP client:
   - Set `[features].rmcp_client = true` in `config.toml` **or** run `zee --enable rmcp_client`
3. Log in with OAuth:
   - `zee mcp login notion`

After successful login, the user will have to restart Zee. You should finish your answer and tell them so when they try again they can continue with Step 1.
...
- Add concise assistant research where helpful: market/industry facts, benchmarks, risks, best practices.
