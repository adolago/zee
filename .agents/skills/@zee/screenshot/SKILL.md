...
3) If Zee needs a screenshot for its own inspection, save to the temp directory.
...
The helpers route Swift's module cache to `$TMPDIR/zee-swift-module-cache`
...
```bash
bash <path-to-skill>/scripts/ensure_macos_permissions.sh && \
python3 <path-to-skill>/scripts/take_screenshot.py --app "Zee"
```

For Zee inspection runs, keep the output in temp:
...
- Temp location (Zee visual check):
...
```bash
python3 <path-to-skill>/scripts/take_screenshot.py --app "Zee" --window-name "Settings"
```

- List matching window ids before capturing (macOS only):

```bash
python3 <path-to-skill>/scripts/take_screenshot.py --list-windows --app "Zee"
```
