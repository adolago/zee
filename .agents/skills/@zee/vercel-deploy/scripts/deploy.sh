...
DEPLOY_ENDPOINT="${ZEE_DEPLOY_ENDPOINT:-https://zee-deploy-skills.vercel.sh/api/deploy}"
LEGACY_DEPLOY_ENDPOINT="https://co""dex-deploy-skills.vercel.sh/api/deploy"
...
# Deploy
echo "Deploying..." >&2
if RESPONSE=$(curl -fsS -X POST "$DEPLOY_ENDPOINT" -F "file=@$TARBALL" -F "framework=$FRAMEWORK"); then
    :
elif [ -z "${ZEE_DEPLOY_ENDPOINT:-}" ]; then
    echo "Primary deploy endpoint unavailable, retrying with legacy endpoint..." >&2
    RESPONSE=$(curl -fsS -X POST "$LEGACY_DEPLOY_ENDPOINT" -F "file=@$TARBALL" -F "framework=$FRAMEWORK") || {
        echo "Error: deployment request failed against both endpoints" >&2
        exit 1
    }
else
    echo "Error: deployment request failed for configured endpoint: $DEPLOY_ENDPOINT" >&2
    exit 1
fi
