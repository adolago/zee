...
struct WebChatSwiftUISmokeTests {
    private struct TestTransport: ZeeChatTransport, Sendable {
        func requestHistory(sessionKey: String) async throws -> ZeeChatHistoryPayload {
            let json = """
            {"sessionKey":"\(sessionKey)","sessionId":null,"messages":[],"thinkingLevel":"off"}
            """
            return try JSONDecoder().decode(ZeeChatHistoryPayload.self, from: Data(json.utf8))
        }
            ...
            attachments _: [ZeeChatAttachmentPayload]) async throws -> ZeeChatSendResponse
        {
            ...
            """
            return try JSONDecoder().decode(ZeeChatSendResponse.self, from: Data(json.utf8))
        }
