...
@Suite(.serialized) struct UtilitiesTests {
    ...
    @Test func gatewayEntrypointPrefersDistOverBin() throws {
        ...
        let bin = tmp.appendingPathComponent("bin/zee.js")
    ...
    @Test func logLocatorPicksNewestLogFile() throws {
        ...
        let dir = URL(fileURLWithPath: "/tmp/zee", isDirectory: true)
        try? fm.createDirectory(at: dir, withIntermediateDirectories: true)

        let older = dir.appendingPathComponent("zee-old-\(UUID().uuidString).log")
        let newer = dir.appendingPathComponent("zee-new-\(UUID().uuidString).log")
