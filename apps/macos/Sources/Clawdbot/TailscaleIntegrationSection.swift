...
struct TailscaleIntegrationSection: View {
    ...
    private var authFields: some View {
        ...
        Text("Stored in ~/.zee/zee.json. Prefer ZEE_GATEWAY_PASSWORD for production.")
            .font(.caption)
            .foregroundStyle(.secondary)
    ...
    private func applySettings() async {
        ...
        if self.connectionMode == .local, !self.isPaused {
            self.statusMessage = "Saved to ~/.zee/zee.json. Restarting gateway…"
        } else {
            self.statusMessage = "Saved to ~/.zee/zee.json. Restart the gateway to apply."
        }
