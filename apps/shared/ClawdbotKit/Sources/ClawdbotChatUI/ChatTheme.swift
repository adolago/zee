...
enum ZeePlatformImageFactory {
    static func image(_ image: ZeePlatformImage) -> Image {
        #if os(macOS)
        Image(nsImage: image)
        #else
        Image(uiImage: image)
