...
struct IOSBridgeChatTransport: ZeeChatTransport, Sendable {
    ...
    func listSessions(limit: Int?) async throws -> ZeeChatSessionsListResponse {
        ...
        return try JSONDecoder().decode(ZeeChatSessionsListResponse.self, from: res)
    }
    ...
    func requestHistory(sessionKey: String) async throws -> ZeeChatHistoryPayload {
        ...
        return try JSONDecoder().decode(ZeeChatHistoryPayload.self, from: res)
    }
        ...
        attachments: [ZeeChatAttachmentPayload]) async throws -> ZeeChatSendResponse
    {
        struct Params: Codable {
            ...
            var message: String
            var thinking: String
            var attachments: [ZeeChatAttachmentPayload]?
            var timeoutMs: Int
        ...
        return try JSONDecoder().decode(ZeeChatSendResponse.self, from: res)
    }

    func requestHealth(timeoutMs: Int) async throws -> Bool {
        let seconds = max(1, Int(ceil(Double(timeoutMs) / 1000.0)))
        let res = try await self.bridge.request(method: "health", paramsJSON: nil, timeoutSeconds: seconds)
        return (try? JSONDecoder().decode(ZeeGatewayHealthOK.self, from: res))?.ok ?? true
    }
            ...
            let task = Task {
                ...
                for await evt in stream {
                    ...
                    case "health":
                        ...
                        let ok = (try? JSONDecoder().decode(ZeeGatewayHealthOK.self, from: data))?.ok ?? true
                        continuation.yield(.health(ok: ok))
                    case "chat":
                        guard let json = evt.payloadJSON, let data = json.data(using: .utf8) else { break }
                        if let payload = try? JSONDecoder().decode(ZeeChatEventPayload.self, from: data) {
                            continuation.yield(.chat(payload))
                        }
                    case "agent":
                        guard let json = evt.payloadJSON, let data = json.data(using: .utf8) else { break }
                        if let payload = try? JSONDecoder().decode(ZeeAgentEventPayload.self, from: data) {
