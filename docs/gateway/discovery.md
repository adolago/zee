...
- Service types:
  - `_zee-bridge._tcp` (bridge transport beacon)
- TXT keys (non-secret):
  - `role=gateway`
...
- `ZEE_SSH_PORT` overrides the SSH port advertised in the bridge beacon (defaults to 22).
- `ZEE_TAILNET_DNS` publishes a `tailnetDns` hint (MagicDNS) in the bridge beacon (auto-detected if unset).
