...
- Pi-mono: latest upstream tag `v0.53.1`; installed pin currently `unknown` (no discoverable `@mariozechner/pi-coding-agent` dependency in known manifests).
...
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| 1 | openclaw | `openclaw/openclaw#15035` | security/reliability | adapt | in-progress (local) | Brute-force resistance and auth endpoint hardening for gateway control plane. | `packages/zee/Swabble/src/gateway/auth-rate-limit.ts` | `bun test packages/zee/Swabble/src/gateway/auth-rate-limit.test.ts` |
| 2 | openclaw | `openclaw/openclaw#13184` | security | adapt | todo | Loopback-by-default binding reduces accidental external exposure in standalone channel/server surfaces. | `packages/zee/Swabble/src/gateway`, `packages/zee/src/cli/cmd/serve.ts` | Add bind-default tests + `./scripts/validate-sync.sh` |
| 3 | openclaw | `openclaw/openclaw#10529` | security | adapt | todo | Enforcing strict credential file permissions prevents local secret disclosure. | `packages/zee/Swabble/src/security`, credential storage paths | Unit tests for mode checks + Linux integration smoke |
| 4 | pimono | dependency pin hygiene | security/ops | port | todo | Unknown installed pin blocks safe update decisions and auditability. | `scripts/lib/upstream-common.sh`, `packages/zee/src/compare/snapshot.ts` | `./scripts/check-upstream.sh --remote pimono --fetch --verbose`, compare warning resolved |
| 5 | openclaw | `openclaw/openclaw#15195` | reliability | adapt | in-progress (local) | Process guard improvements reduce runtime wedging and orphaned subprocess behavior. | `packages/zee/src/cli/cmd/runtime-process-guard.ts`, daemon/runtime orchestration | runtime guard tests + daemon restart smoke |
| 6 | openclaw | `openclaw/openclaw#13746` | reliability | port | todo | Avoid flushing pending tool results before agents are idle to prevent dropped outputs. | `packages/zee/Swabble/src/agents`, queue/flush lifecycle | regression test for flush ordering |
| 7 | openclaw | `openclaw/openclaw#13578` | reliability | port | todo | Prevent silent `allowList[0]` fallback misrouting in outbound messaging. | WhatsApp/plugin routing in `packages/zee/Swabble/src/channels` | route-selection tests (error on unresolved target) |
| 8 | openclaw | `openclaw/openclaw#14949` | reliability | adapt | todo | Transcript archival on `/new` and `/reset` prevents history loss and improves operability. | session/transcript persistence in Swabble runtime | lifecycle test: reset/new preserves archive |
| 9 | openclaw | `openclaw/openclaw#12846` | reliability | adapt | todo | Session-key normalization prevents duplicate/ghost sessions. | session key derivation and storage keys | key-normalization regression tests |
| 10 | openclaw | `openclaw/openclaw#15323` | reliability | adapt | todo | Canonical absolute `sessionFile` handling improves compatibility and deterministic storage. | session path resolution stack | path normalization tests (relative/absolute) |
| 11 | openclaw | `openclaw/openclaw#15154` | reliability | adapt | todo | Fixing transcript path resolution for non-default agents prevents split or missing logs. | multi-agent transcript path resolver | agent-variant transcript tests |
| 12 | openclaw | `openclaw/openclaw#15573` | reliability | adapt | todo | Preserve streamed text when final payload regresses to avoid output truncation. | streaming assembly in Swabble runtime | stream/final reconciliation regression test |
| 13 | openclaw | `openclaw/openclaw#14498` | reliability | adapt | todo | Promise-chain mutex model reduces session-store race conditions. | session store write coordination | concurrency stress test |
| 14 | openclaw | `openclaw/openclaw#15642` | reliability | port | todo | Preserve Windows backslashes in command parsing for cross-platform correctness. | Windows command parsing in daemon/service paths | Windows path parsing tests |
| 15 | openclaw | `openclaw/openclaw#11547` | reliability | port | todo | Preserve literal `\\n` in inbound Windows path/text flows. | inbound parsing/text normalization | literal escape-preservation tests |
...
| 17 | openclaw | `openclaw/openclaw#14976` | reliability | adapt | verify | Confirm `replyToCurrent` parity path works end-to-end. | reply mode injection and routing paths | E2E reply-mode parity scenario |
