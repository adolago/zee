...
| --- | --- | --- | --- | --- | --- |
| 01 | commit `ec45c317f5` | security | port | Trusted-proxy bypass in gateway control-path is a high-risk auth boundary issue. | TODO: audit Zee gateway trusted-proxy/auth chain and add bypass regression coverage. |
| 01 | commit `c736f11a16` | security | adapt | Browser WebSocket auth hardening is parity-critical for gateway control-plane exposure. | TODO: compare browser WS auth flow with upstream hardening and port missing checks. |
...
| 07 | commit `04d91d0319` | security | port | Workspace hardlink alias escapes are filesystem boundary bypasses. | TODO: add hardlink alias escape guard in workspace/path policy layer with tests. |
| 07 | commit `125f4071bc` | security | port | `agents.files` symlink escape blocking is core path-safety hardening. | TODO: verify/port symlink escape denial in agents.files handlers and add fixture coverage. |
