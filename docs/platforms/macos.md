---
summary: "Spec for the Zee macOS companion menu bar app (gateway + node broker)"
read_when:
  - Implementing macOS app features
  - Changing gateway lifecycle or node bridging on macOS
---
# Zee macOS Companion (menu bar + gateway broker)
...
If you run the bundled macOS app, it installs a per-user LaunchAgent labeled `com.zee.gateway`.
CLI-only installs can use `zee onboard --install-daemon`, `zee daemon install`, or `zee configure` → **Gateway daemon**.

```bash
launchctl kickstart -k gui/$UID/com.zee.gateway
launchctl bootout gui/$UID/com.zee.gateway
```

`launchctl` only works if the LaunchAgent is installed; otherwise run `zee daemon install` first.

Details: [Gateway runbook](/gateway) and [Bundled bun Gateway](/platforms/mac/bun).

## Purpose
- Single macOS menu-bar app named **Zee** that:
  - Shows native notifications for Zee/zee events.
  ...
  - Installs a single CLI (`zee`) by symlinking the bundled binary.

## High-level design
- SwiftPM package in `apps/macos/` (macOS 15+, Swift 6).
- Targets:
  - `ZeeIPC` (shared Codable types + helpers for app‑internal actions).
  - `Zee` (LSUIElement MenuBarExtra app; hosts Gateway + node bridge + PeekabooBridgeHost).
- Bundle ID: `com.zee.mac`.
- Bundled runtime binaries live under `Contents/Resources/Relay/`:
  - `zee` (bun‑compiled relay: CLI + gateway)
- The app symlinks `zee` into `/usr/local/bin` and `/opt/homebrew/bin`.

## Gateway + node bridge
- The mac app runs the Gateway in **local** mode (unless configured remote).
- The gateway port is configurable via `gateway.port` or `ZEE_GATEWAY_PORT` (default 18789). The mac app reads that value for launchd, probes, and remote SSH tunnels.
...
- The mac app watches `~/.zee/zee.json` and switches modes live when `gateway.mode` or `gateway.remote.url` changes.
...
## CLI (`zee`)
- The **only** CLI is `zee` (TS/bun). There is no `zee-mac` helper.
- For mac‑specific actions, the CLI uses `node.invoke`:
  - `zee nodes canvas present|navigate|eval|snapshot|a2ui push|a2ui reset`
  - `zee nodes run --node <id> -- <command...>`
  - `zee nodes notify --node <id> --title ...`
...
Zee (the macOS app) registers a URL scheme for triggering local actions from anywhere (browser, Shortcuts, CLI, etc.).
...
```bash
open 'zee://agent?message=Hello%20from%20deep%20link'
```
...
- With `key=<value>`, Zee runs without prompting (intended for personal automations).
  - The current key is shown in Debug Settings and stored locally in UserDefaults.

Notes:
- In local mode, Zee will start the local Gateway if needed before issuing the request.
- In remote mode, Zee will use the configured remote tunnel/endpoint.

## Build & dev workflow (native)
- `cd apps/macos && swift build` (debug) / `swift build -c release`.
- Run app for dev: `swift run Zee` (or Xcode scheme).
- Package app + CLI: [`scripts/package-mac-app.sh`](https://github.com/zee/zee/blob/main/scripts/package-mac-app.sh) (builds bun CLI + gateway).
