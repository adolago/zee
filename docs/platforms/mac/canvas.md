...
Zee can embed an agent-controlled “visual workspace” panel (“Canvas”) inside the macOS app using `WKWebView`, served via a **custom URL scheme** (no loopback HTTP port required).
...
- `zee-canvas://<session>/<path>`

Routing model:
- `zee-canvas://main/` → `<canvasRoot>/main/index.html` (or `index.htm`)
- `zee-canvas://main/yolo` → `<canvasRoot>/main/yolo/index.html` (or `index.htm`)
- `zee-canvas://main/assets/app.css` → `<canvasRoot>/main/assets/app.css`
...
- `~/Library/Application Support/Zee/canvas/<session>/…`

This keeps it alongside other app-owned state and avoids mixing with `~/.zee/` gateway config.
...
This should be modeled after `WebChatManager`/`WebChatSwiftUIWindowController` but targeting `zee-canvas://…` URLs.

Related:
- For “invoke the agent again from UI” flows, prefer the macOS deep link scheme (`zee://agent?...`) so *any* UI surface (Canvas, WebChat, native views) can trigger a new agent run. See [`docs/macos.md`](/platforms/macos).

## Agent commands (current)

Use the main `zee` CLI; it invokes canvas commands via `node.invoke`.

- `zee nodes canvas present --node <id> [--target <...>] [--x/--y/--width/--height]`
...
- `zee nodes canvas hide --node <id>`
- `zee nodes canvas eval --js <code> --node <id>`
- `zee nodes canvas snapshot --node <id>`
...
```
http://<gateway-host>:18793/__zee__/a2ui/
```
...
- `zee nodes canvas a2ui push --jsonl <path> --node <id>`
- `zee nodes canvas a2ui reset --node <id>`
...
zee nodes canvas a2ui push --jsonl /tmp/a2ui-v0.8.jsonl --node <id>
```
...
- Quick smoke: `zee nodes canvas a2ui push --node <id> --text "Hello from A2UI"` renders a minimal v0.8 view.
...
This is intentionally separate from `zee-canvas://…` (which is only for serving local Canvas files into the `WKWebView`).

Suggested patterns:
- HTML: render links/buttons that navigate to `zee://agent?message=...`.
- JS: set `window.location.href = 'zee://agent?...'` for “run this now” actions.

Implementation note (important):
- In `WKWebView`, intercept `zee://…` navigations in `WKNavigationDelegate` and forward them to the app, e.g. by calling `DeepLinkHandler.shared.handle(url:)` and returning `.cancel` for the navigation.

Safety:
- Deep links (`zee://agent?...`) are always enabled.
...
- Navigation policy: allow only `zee-canvas://…` (and optionally `about:blank`); open `http/https` externally.
...
- Log scheme requests + resolution decisions to OSLog (subsystem `com.zee`, category `Canvas`).
