...
---
# Zee gateway as a child process of the macOS app
...
## Goal
Run the Node-based Zee/zee gateway as a direct child of the LSUIElement app (instead of a launchd agent) while keeping all TCC-sensitive work inside the Swift app/broker layer and wiring the existing “Zee Active” toggle to start/stop the child.

## When to prefer the child-process mode
- You want gateway lifetime strictly coupled to the menu-bar app (dies when the app quits) and controlled by the “Zee Active” toggle without touching launchd.
...
- Add a small `GatewayProcessManager` (Swift) that owns:
  ...
  - `start(config)` called when “Zee Active” flips ON:
    - binary: host Node running the bundled gateway under `Zee.app/Contents/Resources/Gateway/`
    - args: current zee entrypoint and flags
    - cwd/env: point to `~/.zee` as today; inject the expanded PATH so Homebrew Node resolves under launchd
    - output: stream stdout/stderr to `/tmp/zee-gateway.log` (cap buffer via Subprocess OutputLimits)
...
- Stream child stdout/stderr to `/tmp/zee-gateway.log`; surface the last N lines in the Debug tab.
...
- Respect the existing pause semantics: when paused, the broker should return `ok=false, "zee paused"`; the gateway should avoid calling privileged routes while paused.
...
- Implement `GatewayProcessManager` with Swift Subprocess to start/stop the gateway on the “Zee Active” toggle.
