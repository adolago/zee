---
summary: "Plan for integrating Peekaboo automation into Zee via PeekabooBridge (socket-based TCC broker)"
read_when:
  - Hosting PeekabooBridge in Zee.app
  - Integrating Peekaboo as a submodule
  - Changing PeekabooBridge protocol/paths
---
# Peekaboo Bridge in Zee (macOS UI automation broker)
...
- Zee integrates by **optionally hosting the same bridge** inside **Zee.app** (user-toggleable). The primary client is the **`peekaboo` CLI** (installed via npm); Zee does not need its own `ui …` CLI surface.
- For **visualizations**, we keep them in **Peekaboo.app** (best UX); Zee stays a thin broker host. No visualizer toggle in Zee.

Non-goals:
- No auto-launching Peekaboo.app.
- No onboarding deep links from the automation endpoint (Zee onboarding already handles permissions).
- No AI provider/agent runtime dependencies in Zee (avoid pulling Tachikoma/MCP into the Zee app/CLI).

## Big refactor (Dec 2025): XPC → Bridge
Peekaboo’s privileged execution moved from “CLI → XPC helper” to “CLI → socket bridge host”. For Zee this is a win:
- It matches the existing “local socket + codesign checks” approach.
- It lets us piggyback on **either** Peekaboo.app’s permissions **or** Zee.app’s permissions (whichever is running).
...
- Zee: `~/Library/Application Support/zee/bridge.sock`

No auto-launch: if a host isn’t reachable, the command fails with a clear error (start Peekaboo.app, Claude.app, or Zee.app).
...
- **Host side (Zee.app)**: `PeekabooBridgeHost` + the minimal Peekaboo services needed to implement operations.

What Zee should *not* embed:
...
## IPC / CLI surface
### No `zee ui …`
We avoid a parallel “Zee UI automation CLI”. Instead:
- `peekaboo` is the user/agent-facing CLI surface for automation and capture.
- Zee.app can host PeekabooBridge as a **thin TCC broker** so Peekaboo can piggyback on Zee permissions when Peekaboo.app isn’t running.
...
- Zee.app converts to global CG coordinates using `NSScreen.screens[screenIndex].frame.origin`.
...
2. Host `PeekabooBridgeHost` inside Zee.app behind a single setting (“Enable Peekaboo Bridge”, default on).
3. Ensure Zee hosts the bridge at `~/Library/Application Support/zee/bridge.sock` and speaks the PeekabooBridge JSON protocol.
4. Validate with `peekaboo bridge status --verbose` that Peekaboo can select Zee as the fallback host (no auto-launch).
