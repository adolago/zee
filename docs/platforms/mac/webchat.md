...
- Auto-open for testing: run `dist/Zee.app/Contents/MacOS/Zee --webchat` (or pass `--webchat` to the binary launched by launchd). The window opens on startup.
- Logs: see [`./scripts/clawlog.sh`](https://github.com/zee/zee/blob/main/scripts/clawlog.sh) (subsystem `com.zee`, category `WebChatSwiftUI`).

## How it’s wired
- Implementation: [`apps/macos/Sources/Zee/WebChatSwiftUI.swift`](https://github.com/zee/zee/blob/main/apps/macos/Sources/Zee/WebChatSwiftUI.swift) hosts `ZeeChatUI` and speaks to the Gateway over `GatewayConnection`.
