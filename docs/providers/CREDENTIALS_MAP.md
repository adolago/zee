...
# Acquire a free OpenBB-compatible provider key
zee auth acquire fred

# List supported free or free-registration OpenBB providers
zee auth acquire --free-only
...
- `zee auth acquire` stores compatible OpenBB credentials in both Zee's auth store and OpenBB `user_settings.json`.
- `zee auth acquire --free-only` only offers providers with free or free-registration access and excludes paid/free-trial-only providers.
